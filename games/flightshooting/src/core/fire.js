...
//  - 9~40단계: 8발 고정. beam = front-8 만큼 탄이 '레이저처럼' 길고 굵어진다(관통 없음). 데미지도 완만 상승.
//  - 여러 발은 부채로 벌어지지 않고 laneGap 간격으로 가로로 나란히 곧게 직진한다(xOff = 중앙 기준 가로 위치).
// 반환: { bullets: [{xOff(가로 위치), beam(레이저 강화 레벨), dmg}], r }.
export function frontSpec(front) {
  ...
  const beam = Math.max(0, front - 8);                   // 8발 채운 뒤 레이저 강화 레벨(길이·굵기·데미지)
  const dmg = baseDmg + Math.floor(beam * F.beamDmg);
  const bullets = [];
  for (let i = 0; i < shots; i++) {
    const xOff = shots === 1 ? 0 : (-(shots - 1) / 2 + i) * F.laneGap; // 중앙 기준 나란히
    bullets.push({ xOff, beam, dmg });
  }
  return { bullets, r };
}
...
export function playerFire(game) {
  ...
  for (const b of spec.bullets) {
    game.bullets.push({
      ...
      r: spec.r, dmg: b.dmg, kind: 'main', beam: b.beam, // beam으로 view가 빔 길이·굵기 분기
    });
  }
  game.sfx.push('shoot');
}
