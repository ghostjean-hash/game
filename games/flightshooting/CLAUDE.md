...
3.1. **전진 방향**: 위쪽(-y). 플레이어는 화면 하단(yRatio 0.82), 적은 위(-r)에서 등장해 아래로 낙하. 아군 탄 vy<0, 적탄은 아래로. 좌표는 CSS 픽셀(stage clientW/H).
3.2. **조작**: 캔버스 드래그(상대 이동 - 잡은 지점 대비 델타를 기체에 적용, 손가락 가림 방지) + 키보드 방향키/WASD. 자동발사(0.14s).
3.3. **화력(power) 1~20**: `CFG.fireAngles`(genFireAngles로 1~20단계 생성). 레벨↑ = 정면 부채 촘촘·확산(최대 11발) + 측면(≥8)·후측면(≥12)·후방(≥16) 순차 추가. 정면 오프셋 0=위쪽.
...
3.7. **보스**: 화면 상단 자리 + 좌우 유영 + 2패턴(아래 방향 부채 산탄 / 조준 3연발) 번갈아. HP = baseHp + (구역-1)*hpPerStage.
