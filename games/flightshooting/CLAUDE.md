...
- 2026-07-21: 홈·인게임 UI 대개편 + 무기 6발/6대 재조정. (1) 홈 전 비율(9:16·3:4·4:3·16:9) fit 재설계 - 스크롤 배제(게임은 스크롤 UI 안 씀), 간격·크기 화면높이 비례(clamp) + 가로모드 좌우 2열(`@media (orientation: landscape)`), 4비율 시작·환경설정 버튼 노출 PASS. (2) 화면 아이콘 문자 전부 SVG화 - 뒤로가기 화살표(메뉴·게임)·치트 렌치·보스 방패(CSS data-uri)·난이도 체크·치트 접기 chevron(classList 토글)·지도 확대축소, 모달 ★ 텍스트 제거. (3) 게임 HUD 진짜 투명 - `.game-screen .topbar`를 캔버스 위 `position:absolute` 오버레이 + `.game-screen position:relative`로 캔버스 전체높이 확장(뒤 게임 비침). 1차 배경만 `transparent`는 topbar가 세로공간 차지해 남색 띠 잔존 → 오버레이로 재수정. (4) HUD 정리 - 지역명(`#hud-loc`) 삭제 구역만, 아이템 배지(P/S/E/T) 아래 강화숫자 세로배치(`.mh.row` column) + 70% 축소·가변폭 자간압축(로마숫자 폭 절약). (5) 스피커·전체화면 홈 우상단 이동(인게임 HUD 제거)+홈 상단바 여백 축소로 fit 회수. (6) 메인총알 8→6발·사이드 옵션기 8→6대·발별진화 6스텝(`front.maxShots`/`option.maxPerSide 3` CFG화, `setFrontHud`/`setEvoHud` 티어계산 8→6 누락 수정 포함), core 124 PASS + 발사 캡처, docs 01·02·05·07 6발 동기화(에이전트 위임). (7) 월드맵 - 지도 풀사이즈 + 제목/힌트 알약 오버레이(세로 스택 해체), 도착카드 화면중앙→도시 위 표시(`placeCardOverCity` getScreenCTM, 상단 도시는 자동 아래로), 발리 표기 인도네시아(위)·발리(아래) 순서(travel 스왑), "골라라"→"골라주세요". (8) 인게임 배너 나라/도시 점→줄바꿈(`banner-country`/`banner-city` 분리) + 나라명 20%↓ 하늘색·검은 외곽선·도시 검은 외곽선. (9) 보스 격파 폭발 2초 통일(`bossDeath.dur`/`finalDur` 2). 미해결(사용자 대기): 배너 도시>나라 크기 조정 / 월드맵 카드 중앙 체감(캐시 의심, 강제 새로고침 안내) / HUD 글자 그림자 / 긴 로마자 티어 확인 / 배너 도시 크기.
- 2026-07-21(배포): 위 개편을 GitHub Pages에 배포 완료(원격 rebase 후 push, 94fe38b). 이전 "배경 검수 후 배포" durable 보류는 배경 검수가 지난 세션에 끝나 해제한다 - 앞으로 `/jarvis-checkpoint`는 규칙대로 커밋 + 배포(push)까지 자동 진행한다(이번 봉합에서 durable 보류를 과잉 적용해 push를 멈췄던 것을 정정). 비-프로그래머 사용자에게 배포 3단계(이 컴퓨터 수정 = 로컬 / 커밋 = 저장 도장 / 푸시 = 인터넷 공개) 개념을 안내했고, 로컬(dev-server)에서 최신으로 보여도 push 전에는 공개 URL·타 기기에 반영되지 않는다는 점을 확인.
- 2026-07-24: 디오라마 배경 전 국가 확장 + 월드맵 목적지 개방 + 배경 위 가독성 개편. (1) 배경 자산 구조 평평화(`assets/diorama/` 아래 `국가코드-도시명.png`, 폴더 없음), `KR-seoul` 개명, `view.js` 경로. (2) `DIORAMA_SRC` 58개 전체 매핑(파일 없으면 우주 배경 fallback), `drawDiorama`를 세로기준→**가로 fit**(좌우 꽉참, 아이패드처럼 넓으면 확대), `DIORAMA_ZOOM` 제거. (3) 사용자가 배경 이미지 17개 투입 → 전부 연결. 대만 철자 불일치(`TW-taibei`→`TW-taipei`) 정정. (4) `docs/14_diorama-city-list.md` 신설 - 수도(나라 상징 아님) 기준 58 도시 리스트 = ChatGPT 제작 지시서, `backgroundPools` 나라상징 방식 폐기 방향. (5) 월드맵: 다음 목적지 순번 제한 제거→**안 간 나라 전부 개방**(못 가는 나라 방지) + 초기 확대는 가까운 `frameNear`(6)개만 감싸 이름 가독, 국가명 라벨 확대, 배경 있는 도시 **금색 별(★)** 표시(`DIORAMA_READY` 17개국). (6) 아이패드 지도 두 손가락 **핀치줌**(pointer 2개 거리비율). (7) 배경 위 가독성: 개수 적은 큰 요소(적·비행기)에만 **밝은 크림 후광**(어두운 배경 대응, 성능 위해 총알 제외 - `shadowBlur`가 총알 수십 개면 버벅), 키위 **어두운 윤곽선**(밝은 크림은 "키위답지 않다" 번복), 적탄 어두운 윤곽선. `entityShadow`/`entityShadowBlur`. (8) favicon 404 해소(`<link rel=icon>`). (9) 제주도·뉴욕 여행지 추가(54국+4여행지=58). browser-shot 검증(한국 배경 좌우꽉참·지도 금색별 다국·키위 윤곽·제주 핀, 콘솔 0). 미해결: 핀치줌/후광 아이패드 실측, 아군 총알 가독성, 총알 png·오프스크린 최적화(버벅일 때), `backgroundPools` 코드 제거(디오라마 다 채운 뒤), 나머지 41개국 배경 미제작.
- 2026-07-25: 직전 세션 미커밋 flightshooting 작업 4건 봉합(코드는 직전 세션 완성, 커밋만 누락돼 있던 것을 이번 /jc로 정리). (1) 자동 플레이 점수 감산 - `core/score.js` 신설(`awardScore`/`isAutoControlling`), 자동 플레이가 **실제 조종 중일 때만** 획득 점수 1/3(`CFG.score.aiDivisor` 3), 손으로 조작 중이면 원점수. 정수 나눗셈 잔여를 `scoreRemainder`로 누적해 합계가 정확히 1/3이 되게 함(부동소수점 오차 0). 점수 가산 6곳(존 처치·부위 파괴·최대강화 보너스·봄 전멸·충돌 처치·보스 격파)을 `awardScore`로 통일, main.js 상태·저장·복원에 `scoreRemainder` 추가. (2) 보통 난이도 재조정 + 초반 연발 제한 - normal `enemyFireMul` 1→1.15·`enemyHpMul` 1→0.95·`radialMul` 1→0.9로 소폭 완화 + `earlyShots`{throughStage 10, max 2} 신설로 1~10구역 조준 연발 최대 2발(`world.enemyShotCap` 헬퍼, updateEnemies·updateBoss 공용). (3) 보스 사망 연출·목적지 전환 대기 - 보스 폭발 파티클(`tag:'bossDeath'`)이 전부 사라진 뒤에만 월드맵 전환, 파티클이 비정상적으로 남으면 `maxDur` 3초에 강제 정리(`stepBossDeath` 재작성, `spawn.burst` tag 파라미터, `bossDeath.dur` 2→1·`bossDeathTime` 제거). 도시 선택 후에는 다음 도시 디오라마 배경 로드 완료(없으면 fallback 확정)까지 `map-loading` 상태로 지도를 유지하고 전투를 시작하지 않음(`closeMapAndAdvance` async, `view.preloadDiorama`·`getDiorama` ready Promise + onerror). (4) 월드맵 금색 별 위치 - 점 우상단→도시 터치 원 안 중앙으로 겹쳐 표시(`bgRing`→`bgStarScale`), 미방문 도시에만 표시(`visitedSet` 제외), 별에 `pointer-events:none`로 도시 선택 클릭 보존. 검증: core 124/124 PASS(browser-shot, 신규 score.js import 체인 무결 + 회귀 0). docs 01·02·06·10 동기화. 미해결: 스크립트 더블클릭 배치(서버켜기·저장하고올리기) 제안 - 사용자 커밋 메시지 방식 미결정 / AGENTS.md(Codex 지침 복제본) untracked - 정체 미확인이라 손대지 않음.
- 2026-07-25(후속): 위 봉합 커밋(e407a9e) 직후 working tree에 뒤늦게 나타난 직전 세션 작업 2건 추가 봉합(IDE 지연 저장/병렬 편집 의심). (1) 보통 난이도 한 줄 웨이브 최대 5마리 제한 - normal에 `waveMax` 5 신설, `buildWaves(stage, waveMax=Infinity)`로 파라미터화(기존 분기를 `buildBaseWaves`로 분리 + 초과분을 잘라 가로 균등 재배치해 왼쪽만 비는 줄 방지), `world.startStage`가 `game.waveMax` 전달, main.js 상태 세팅. (2) 월드맵 별을 글꼴(★ text)→SVG polygon 도형으로 교체 - 글꼴 세로 기준점 차이 없이 별 중심이 도시 터치 원 중심에 정확히 오게 함(`mapStarPoints` 헬퍼), 색을 금색→시안/회청 버튼 위에 잘 보이는 짙은 남청(`bgReady`) + 밝은 윤곽(`bgReadyStroke`)으로 대비 강화, `bgStarScale`→`bgStarOuterScale`/`bgStarInnerScale`/`bgStarPoints`/`bgStarStroke`. 검증: core 124/124 PASS(browser-shot, buildWaves 시그니처 변경 기존 호출 호환). docs 01·02·10 동기화.
- 2026-08-01: html-game v0.4.2 적용(4.8 시작 흐름 공용 프레임 3단계). 전용 메뉴 화면의 항목이 규격이 정한 같은 자리로 옮겨졌다 - 난이도 넷은 한 줄 세그, 친구 동행·자동 플레이는 시작 바로 위 켜고 끄는 줄(공용 부품에 `toggles` 신설, 규격 4.8-16), 이어서 하기는 그 위 칸, 환경설정은 상단 띠 톱니. 게임이 직접 쌓던 이동 기록과 뒤로가기 가로채기를 걷어내고 공용 계단 하나만 남겼다(규격 4.8-18 - 그대로 두면 뒤로가기 한 번에 두 칸이 물러난다). 결과·일시정지가 덮는 카드로 통일. 플레이 화면은 HUD를 캔버스 위에 겹쳐 놓은 구조라 그 화면에서만 공용 띠를 감추고 소리 버튼을 HUD에 둔다(규격 4.8-17). 소리 모듈은 이 게임 전용 후처리 그래프(컴프레서·리버브)만 남기고 그릇은 공용으로. 적용 중 실측 사고 하나 - 프레임이 미리 만든 빈 화면 자리가 남아 게임 화면과 높이를 반씩 나눠 가져 보드가 절반이 됐다. 공용 `register`가 같은 이름의 앞 자리 이름표를 떼도록 고쳤다. core 146/146 PASS.
