...
#### 1.1.1. 만렙(P8) 이후 탄 모양 진화 (사용자 확정 2026-07-08)
- 전방 화력이 8(만렙)에 닿은 뒤 P를 더 먹으면, 탄 수(8발)는 고정한 채 **탄 모양이 4단계로 진화**한다. 만렙 후 P 획득이 점수로만 처리돼 성장감이 끊기던 공백을 시각·위력 성장으로 메운다.
- 모양 순서(`game.shapeTier` 1~4, 0=진화 전 기본 세로 타원): **원 → 타원 → 긴형 → 링**. 갈수록 굵거나 길어져 존재감이 단조 증가하고, 마지막 링(고리)만 형태 축을 바꿔 최종·최강을 표시한다.
  - 1 원: 기본 타원보다 굵고 큰 동그란 광탄
  - 2 타원: 그 원을 세로로 늘인 럭비공형
  - 3 긴형: 레이저빔처럼 더 길게 늘인 탄
  - 4 링: 가운데가 빈 발광 고리탄(최종)
- 모양이 한 단계 오를 때마다 **데미지도 `CFG.parts.front.shapeDmg`(기본 1)씩 상승**한다. 탄 히트박스(r)는 P8 기준 유지하고 렌더 크기만 `CFG.bullet.shapes[]` 배율로 키운다(밸런스 안정).
- 모양 4단계까지 다 채운 뒤의 P는 기존대로 `maxedBonus` 점수로 처리한다.
- HUD의 전방 화력 칸은 만렙 시 `★`, 진화하면 `★1`~`★4`로 티어를 함께 보인다.
...
- 획득 순서를 `game.partHistory` 스택에 계통 이름('front'|'shape'|'option'|'zone')으로 기록.
- 피격 시 스택 pop → 해당 계통 1단계 되돌림(front -1(최소 1)/shape -1(최소 0, 모양 진화 1단계 후퇴)/option 마지막 1대 제거/zone -1). front 시작값 1은 스택에 없음(P로 2 이상 올릴 때만 push). 만렙 후 얻은 모양 진화는 'shape'로 push되어 front보다 먼저(역순) 손실된다.
- 회복(H)·봄(B)은 기존 유지.
