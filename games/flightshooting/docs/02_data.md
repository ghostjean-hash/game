...
- 동행 키위새는 `assets/characters/kiwi-upward-v3.png`로 표시하며, 충돌 반경은 유지한 채 화면 표시·체력 점·말풍선 기준 크기를 1.5배로 렌더한다. 보스 코어는 `assets/bosses/`의 battleship/bio/orbiter/sentinel 4종 PNG를 스타일별로 사용하며, 전신이 들어오도록 축소하지 않고 크게 그려 화면 상단에서 후방이 자연스럽게 잘리게 한다. 따라서 플레이어에게 실제 위협이 되는 전면 코어·공격 파츠가 크게 내려온다. 기존 부위 파괴·약점 코어·체력 연출은 그 위에 유지하며, 자산 로드 실패 시 기존 캔버스 도형으로 대체한다.
...
- `enemyHpScale`: 0.28. 실제 hp = `ceil(base × (1 + (stage-1)×scale))` (spawn.js에서 구역별 적용). 도시 라이벌 수호자도 같은 구역·난이도 체력 공식을 쓴다.
...
- `boss`: bobAmp / bobFreq(좌우 유영 폭·속도) / bobRamp / spawnTop / partScale 2.94(부위 기준 원본 크기의 140%) / partHitFlash(코어·부위 PNG 실루엣 백색 피격 플래시 지속) / partScarSparkTime(파괴 흔적의 불꽃 감쇠 시간).
...
- `autopilot`: default('pro' 기본 실력) / resumeDelay(0.5, 하이브리드 자동 복귀 지연 s) / safetyPad(충돌 판정 추가 안전 여백) / shotForecast(발사 예정 탄 예측 지평) / emergencyTime·emergencyReact(충돌 임박 시 반사 회피 기준·주기) / moveAccel·emergencyAccel(자동 이동의 일반/긴급 가속도) / tiers{beginner,intermediate,advanced,pro} 각 react·aimDeg·sim·threats(인간 실측 기준). 자동 AI는 현재 적탄·적 본체·보스 코어/파츠와 함께 발사 타이머가 shotForecast 안인 적·보스 파츠의 예상 조준탄을 평가한다. 일반 판단은 목표를 잠시 유지하고 속도를 부드럽게 전환한다.
- `friend`: `moveAccel`(키위새 전용 가속) / `mateGap`(플레이어와 유지할 거리) / `itemBonus`(플레이어 몫을 제외한 아이템 수집 우선도). 보스전 외에만 적용한다.
