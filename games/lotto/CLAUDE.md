...
- 2026-07-14: 게임명 'Blessed Lotto'→'lotto 추천번호'. 휠링 진입 버그(setTab의 TABS 가드) 수정. 데이터 초기화 안내 유저 언어 재작성. 운세 추첨 - 순수 학설 프리셋은 행운 집합 안에서만(B안) + 풀 안 조작 없는 순수 무작위(고정 가중·대역 편중 모두 미채택, 사용자 결정). 회귀 324 PASS.
