...
캐릭터 속성이 아니라 **추첨 시 사용자가 선택**하는 가중치 정책입니다. 같은 캐릭터로 12가지 전략 모두 시도 가능.
...
| `mbti` | MBTI 행운 | 캐릭터 MBTI 16종의 행운 번호 위주 | **운세 매핑(서양)** | MBTI 행운 번호 5배 boost | 시드 |
...
- **시드 의존(7개)**: `blessed` / `pairTracker` / `astrologer` / `intuitive` / `mbti` / `zodiacElement` / `fiveElements`. 캐릭터 시드 + drwNo + Luck. 캐릭터별 다른 결과. 결정론 시드: `mixSeeds(seed, drwNo)`.
...
| **운세 매핑(서양)** | 서양 점성술 / 성격유형 → 행운 번호 임의 매핑 | 3 | astrologer / mbti / zodiacElement |
...
1.5.2.5. **운세 매핑 / 사주 카테고리 정직성 (S6-T1, 2026-05-02)**: 본 두 카테고리(별자리 / 별자리 4원소 / MBTI / 일주 오행 4종)의 행운 번호는 **게임 디자이너가 임의로 배정한 콘텐츠**이며, 점성술·MBTI·명리학 등 외부 학설과 무관하다. UI 노출 위치 3곳 모두 면책 카피 강제:
- 캐릭터 카드 4종 행운 토글 패널: "임의 매핑 · {분야} 학설과 무관 · 추첨 확률 영향 없음" 캡션 (`render/character-card.js`).
- 전략 탭 활성 desc 아래: 카테고리가 운세 매핑 / 사주일 때 한 줄 면책 (`render/strategy-tabs.js .strategy-mapping-note`).
- 추천 결과 reasons: 4종 전략(astrologer / mbti / zodiacElement / fiveElements)의 reasons에 "(임의 매핑, 추첨 확률 영향 없음)" 접미 강제 (`core/recommend.js`).
...
### 1.13. MBTI 16종 + 행운 번호 (MBTI 전략용, 임의 매핑)

> **출처 명시 (S6-T1, 2026-05-02)**: 본 표의 행운 번호는 게임 디자이너가 임의로 배정한 콘텐츠이며, 16Personalities 등 MBTI 학설과 무관합니다. 캐릭터 정체성 강화 / 콘텐츠 다양성 목적이며, 추첨 확률에는 일체 영향이 없습니다.
