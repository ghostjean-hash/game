...
| ID | UI 라벨 | UI 설명 | 가중치 정책 | 의존성 |
|---|---|---|---|---|
| `blessed` | 축복받은 자 | 모든 번호에서 균등 추출, Luck이 시드 번호 가중치를 강화 | 균등 + Luck 분산 | 시드+Luck |
| `statistician` | 통계 추첨 | 역대 회차에 가장 많이 나온 번호 위주 | 본번호 누적 빈도 가중 | **객관** |
| `secondStar` | 보너스볼 사냥 | 역대 보너스볼로 자주 나온 번호 위주 | 보너스볼 빈도 가중 | **객관** |
| `regressionist` | 미출현 회귀 | 오랫동안 안 나온 번호 위주 | 미출현 갭 가중 | **객관** |
| `pairTracker` | 짝꿍 번호 | 캐릭터 키번호와 자주 함께 나왔던 번호 묶음 | 시드 키번호와 동시출현 페어 가중 | 시드 |
| `astrologer` | 별자리 행운 | 캐릭터 별자리 12종의 행운 번호 위주 | 별자리 행운 번호 5배 boost | 캐릭터 |
| `trendFollower` | 최근 트렌드 | 최근 30회에 자주 나온 번호 위주 | 최근 30회 Hot 번호 가중 | **객관** |
| `intuitive` | 직감 | 회차마다 다른 분포 (같은 캐릭터는 같은 결과) | 매 회차 다른 무작위 가중 (시드 결정론 유지) | 시드 |
| `balancer` | 균형 조합 | 번호 합 121~160 + 홀짝 3:3 필터를 통과한 조합만 | 합 121~160 + 홀짝 3:3 (최대 50회 재추첨) | **객관** |
| `mbti` | MBTI 행운 | 캐릭터 MBTI 16종의 행운 번호 위주 | MBTI 행운 번호 5배 boost | 캐릭터 |
| `zodiacElement` | 별자리 4원소 | 별자리 4원소(불/땅/공기/물) 그룹 행운 번호 | 4원소 그룹별 행운 번호 5배 boost | 캐릭터 |

기본 전략: `blessed` (`STRATEGY_DEFAULT`).

#### 1.5.1. 객관 전략 vs 시드 의존 전략
...
- **시드 의존(6개)**: `blessed` / `pairTracker` / `astrologer` / `intuitive` / `mbti` / `zodiacElement`. 캐릭터 시드 + drwNo + Luck. 캐릭터별 다른 결과. 결정론 시드: `mixSeeds(seed, drwNo)`.
- 분류 위치: `src/data/numbers.js` `OBJECTIVE_STRATEGIES` Set. 분기 위치: `src/core/recommend.js`.
- 트레이드오프: 객관 전략은 사용자가 캐릭터를 바꿔도 결과 동일. 그것이 객관 통계의 본질. 캐릭터 차별화는 시드 의존 6개에서.

UI 톤 가이드: 라벨은 직관적, 설명은 "어떻게 뽑는지"를 한 줄로. UI 줄에 라벨 + 설명이 동시 노출되므로 설명에서 라벨 단어 반복 금지.
