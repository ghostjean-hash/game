...
|---|---|---|---|---|---|
| `blessed` | 랜덤 | 모든 번호에서 균등 추출, Luck이 시드 번호 가중치를 강화 | **랜덤** | 균등 + Luck 분산 | 시드+Luck |
...
- **시드 의존(5개, S34 짝꿍 폐기로 6 → 5)**: `blessed` / `astrologer` / `intuitive` / `zodiacElement` / `fiveElements`. 캐릭터 시드 + drwNo + Luck. 캐릭터별 다른 결과. 결정론 시드: `mixSeeds(seed, drwNo)`.
...
- 새 architecture는 시드 보너스를 +0.5 수치로 직접 가산 (Luck 비례). 1-45 base 1.0의 1.5배 max.
- 옛 `applyLuck` 함수는 `@deprecated`. 새 architecture는 `computeUnifiedWeights` 안에서 직접 처리.
...
| `PRESET_SUBTITLE_MAX` | 20 | 부제 글자 cap |

#### 1.20.2. 기본 프리셋 (`DEFAULT_PRESETS`)

첫 진입 / 데이터 손상 / 사용자 reset 시 주입. **S37(2026-05-08, 사행성 책임)** 갱신.

| id | 라벨 | 부제 | 묶음 |
|---|---|---|---|
| `preset-1` | 균형 | 최신 흐름·운세·직감 한 번에 | `trendFollower` + `astrologer` + `blessed` |
| `preset-2` | 분산파 | 남들이 덜 고르는 조합 | `regressionist` + `intuitive` + `balancer` |
| `preset-3` | 운세파 | 동·서양 운세 합 | `astrologer` + `zodiacElement` + `fiveElements` |
...
| `lotto_char_card_collapsed` | `boolean` | 캐릭터 카드 접힘 학습. 흉일은 main.js에서 강제 펼침. |
...
```js
{
  ...
  subtitle: string,      // 20자 cap. 예: '통계·운세·직감 한 번에'
  strategyIds: string[]  // 묶을 전략 id list (1개 이상 필수)
}
```
...
|---|---|---|
| 통계 | 파랑 (sky) | trendFollower `#0ea5e9` / statistician `#0284c7` / secondStar `#075985` / regressionist `#0c4a6e` (S34 pairTracker 항목 제거) |
