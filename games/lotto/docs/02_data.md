...
#### 1.5.5. 5세트 동시 추천 (S4-T1 신설 / S27 추천 탭 노출 폐기, 2026-05-03)

**S27 변경**: 추천 탭에서 메인 카드 + 5세트 컴팩트 노출 폐기. 본 절의 시드 변형 룰은 추천 리스트의 + 5세트 버튼이 동일하게 재사용 (1.5.8 참조). `options.fiveSets`는 storage에 잔존하나 UI 영향 0.
...
추천 탭 진입 / 캐릭터 전환 시마다 자동 호출 (`renderHome` 안). 다음 회차로 넘어가면 이전 누적은 자동 폐기.
...
### 1.16. ~~백캐스트~~ (S090, 2026-05-17 전면 폐기) + 1.16-A history 등록 cap 신설
...
- 등록 항목 schema: `{ drwNo, numbers, bonus, reasons, strategyIds, strategySources, createdAt, matchedRank, source: 'user', revealed: boolean }`. **S097 (2026-05-19) 신설 `revealed`**: 사용자가 발표 후 결과 reveal을 직접 트리거했는가. 신규 등록 = false. 옛 데이터 (matchedRank 있음, S097 이전 등록) = storage load 시 true 자동 마이그레이션.
...
**S63 (2026-05-10) 부제 필드 폐기**: 추천 탭 슬롯 부제(예: "최신·운세·직감 한 번에")가 사용자에게 "애매한 설명"으로 인지됨. 자비스 정직성 정책상 묶인 실제 전략을 그대로 노출하는 게 맞다. 부제 자리에는 자동 생성된 strategy label list(예: "최신 · 별자리 · 직감")를 표시.
...
3.2.3. `lotto_options.fiveSets`: 5세트 동시 추천 모드 (S4-T1). 기본 false. ~~ON 시 추천 탭 메인 카드 + 컴팩트 4장 노출~~ (S27 추천 탭 노출 폐기). 옵션은 storage에 잔존하나 UI 영향 0. 별도 sprint에서 옵션 자체 폐기 검토. 1.5.5 / 1.5.8 참조.
...
```js
{
  ...
  source: 'user',                           // S090(2026-05-17) 신설. 사용자 명시 등록 출처 표기. 옛 데이터 부재 시 'user' 보수적 자동.
  revealed: boolean                         // S097(2026-05-19) 신설. 발표 후 사용자가 ball reveal 트리거했는가. 신규 등록=false / 옛 데이터=true 자동 마이그레이션.
