...
첫 진입 / 데이터 손상 / 사용자 reset 시 주입.

| id | 라벨 | 부제 | 묶음 |
|---|---|---|---|
| `preset-1` | 균형 | 통계·운세·직감 한 번에 | `statistician` + `astrologer` + `blessed` |
| `preset-2` | 통계파 | 데이터 4축 통합 | `statistician` + `regressionist` + `trendFollower` + `secondStar` |
