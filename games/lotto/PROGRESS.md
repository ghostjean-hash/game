...
### 2.111.18. lotto 추천번호 리브랜딩 + 휠링 진입 버그 + 운세 추첨 풀 제한 (2026-07-14)

게임명을 'Blessed Lotto'에서 'lotto 추천번호'로 변경하고, 사용자가 보고한 두 버그(휠링 진입 불가 / 운세 추천의 오라벨·앞번호 반복)를 진단·수정한 cycle. 데이터 초기화 안내도 개발 메모에서 유저 언어로 재작성. 회귀 324 PASS(B안 회귀 테스트 2건 추가).

#### 2.111.18.1. 결정 / 산출 5 묶음

| # | 사용자 명시 | 자비스 진단 / 옵션 제시 | 사용자 결정 | 산출 자산 |
|---|---|---|---|---|
| 1 | "Blessed Lotto → lotto 추천번호" 이름 변경 | 화면 표시 3곳(허브 카드/브라우저 탭/설정 안내) + 문서 일치 대상 식별 | 확정 | `_registry.json` / `index.html` / `settings-page.js` / `docs/01_spec.md` / `CLAUDE.md` / `README.md` / `tests/test.html` / `scripts/fetch-lotto-draws.bat` / 루트 `CLAUDE.md` |
| 2 | 휠링 동작 확인, 안 되면 비활성화 | 근본원인 = `setTab`이 하단 탭 목록(TABS)에만 있는 화면만 허용하는데, 휠링을 설정 탭 진입으로 옮기며 TABS에서 뺀 뒤 이 가드에 걸린 회귀 버그 | (수정 유지 - 비활성화 지시 미수령) | `src/render/main.js` `setTab` 휠링 예외 1줄 |
| 3 | 데이터 초기화 유저 설명 작성 | 기존 "S090-후속 백캐스트 잔재" 개발 메모 → 두 버튼이 각각 무엇을 지우는지 유저 언어로 | 확정 | `src/render/settings-page.js` 데이터 초기화 섹션 문구 |
| 4 | 운세 추천 "나와서는 안 되는 번호" 버그 | 추첨 자체 정상(범위/중복/충돌 0). 운세 프리셋 추천 42.5%가 행운 번호 아닌데 6번 폴백(`strategyIds[0]`)으로 학설 라벨 강제. A안(전범위+라벨정직) vs B안(풀 제한) 제시 | B안 (풀 제한) | `src/core/recommend.js` `isPureMappingPreset` + 풀 제한, `docs/01_spec.md` 5.1.3.0 절 신설, 회귀 2건 |
| 5 | 앞번호(1·3) 반복 지적, A안 거부 | 원인 = 다중매칭 고정 boost가 여러 학설 겹친 작은 수 과대표집 + 풀 좁음(양자리 18개) | 밀어주기(고정 가중) 제거 | `recommend.js` 고정 boost 제거 → `uniformWeights()` |
| 6 | 대역(0/10/20/30/40번대) 균질 지적 | 원인 = 좁고 등차 분포인 union 구조(빈대역 평균 1.07 < 실제 로또 1.23). 세트별 대역 편중 방식 제안·AskUserQuestion 진행 | "의도 없이 순수 랜덤, 몰아주지 마라" → **대역 편중 미채택**, 조작 0 순수 무작위 확정 | `recommend.js` `uniformWeights()` 풀 안 순수 무작위, `01_spec.md` 순수 무작위 절 |

#### 2.111.18.2. 검증

`node tests/run-node.js` → **324 / 324 PASS** (322 + B안 회귀 2건: 순수 운세=행운집합 밖 0건 / 혼합=전범위 유지). browser-shot - 허브 카드 'lotto 추천번호' 표시, 휠링 티켓/예상비용/보장검증 렌더, 데이터 초기화 안내, 운세 5세트 렌더 확인. 진단 스크립트(scratchpad, repo 밖) - 운세 오라벨 42.5%→0.0%, 앞번호 반복은 풀 18개 수학적 한계로 잔존 정량 확인.

#### 2.111.18.3. 잔여 / 후속

- 휠링 "살릴지 vs 비활성화" 사용자 명시 답 미수령. 수정으로 정상 동작 상태 유지(비활성화 지시 없어 보존). 사용자 확인 시 방향 확정 필요.
- 대역 균질·앞번호 반복은 좁은 union(18~23개)의 순수 무작위 결과. 사용자가 "조작 없는 순수 랜덤"을 명시 선택해 그대로 수용 - 대역 편중, 풀 확대(학설 lucky 확대), A안(전 범위) 모두 미채택. 재론 시 데이터 변경 cycle.

### 2.111.19. 운세 추첨 순수성 실증 + union 구조 한계 진단 (2026-07-14, 코드 변경 0)

직전 봉합(a5510d8) 후 사용자 의심 2건에 대한 진단 전용 cycle. 코드/문서 변경 없음, 실측 근거만 남긴다.

1. **순수 랜덤 실증** (사용자 "진짜 랜덤 맞아? 제약 거는 느낌"): 운세 프리셋 20,000세트에서 union 각 번호 출현 빈도 실측. 변동계수 1.3% = 이론 통계오차(1.2%)와 동일, 풀 밖 출현 0. 특정 번호 몰아주기·제약 없음을 정량 확정. `weightedSample`도 uniform weight면 각 번호 동일 확률 비복원 추출임을 코드 경로로 재확인.

2. **union 끝자리 제약 발견** (사용자가 느낀 "제약"의 정체): 학설 lucky가 등차수열(별자리 10칸/사주 5칸 간격)이라 union 끝자리가 소수로 고정(예 aries/gap = 끝자리 1·3·8·9뿐, 0·2·4·5·6·7 부재). 순수 랜덤이어도 결과 끝자리 패턴이 반복돼 "규칙으로 거른다"는 인상. 추첨이 아닌 원재료(lucky 집합) 구조 문제.

3. **생일 차별화 측정** (사용자 "생일 달라도 같은 숫자?"): 120개 생일 조합 pairwise 평균 겹침 38%, 완전 비겹침(0%) 쌍 존재, 전체 공통번호 0개 → 생일이 크게 다르면(별자리 다름) 풀 차별화됨(반증). 단 같은 별자리(생일 한 달 내)는 별자리+원소 몫(약 14개) 고정 + 사주만 변동이라 겹침 큼. 사용자 관찰은 이 케이스로 추정(미확인).

#### 2.111.19.1. 미해결 잔여

- 끝자리 제약 제거 여부: 없애려면 운세 밖 번호 허용(A안, 사용자 기거부) 필요. 운세 순수성 유지 시 잔존. 사용자 결정 대기.
- 생일 차별화: 사용자가 본 게 계절 차이(별자리 다름)인지 같은 별자리 며칠 차이인지 미확인. 전자인데 같게 나왔다면 재조사 필요(현 측정상 별자리 다르면 차별화되므로 버그 가능성 낮음).
