...
**자비스 자기 점검 (14건째 결손)**:
직전 정정(S090-후속 5)에서 `.saved-set-row.is-registered`에만 padding-left 추가했음 = 정렬 깨뜨림. 시각 강조 위해 한쪽 클래스에만 padding 추가하면 다른 클래스와 정렬 어긋남이 필연. 향후 룰: **확정/미확정 같은 toggle 상태 시각 강조는 background / box-shadow / outline 같은 layout 무영향 속성만 사용. padding/margin 변경 금지**.
