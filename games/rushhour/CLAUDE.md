...
- 2026-08-01: html-game v0.4.2 적용(4.8 시작 흐름 공용 프레임 3단계). 시작 화면이 새로 생겼다 - 그전에는 열면 곧바로 퍼즐이 떴고 상점·진행 맵·소리·설정이 위쪽 아이콘 줄에 있었다. 진행 맵이 시작 다음 칸으로 내려가고(골라 들어가는 형), 상점은 새 칸을 만들지 않고 조작단 맨 아래 줄로, 설정은 상단 띠의 톱니로 갔다. 결과 팝업은 플레이를 덮는 공용 카드가 되고 별·수·콤보·골드는 그 카드 본문 조각으로 들어간다(규격 4.8-13). 시작 화면 배경은 실제 48번 문제의 차 배치와 크림 판·출구 길. 소리 그릇은 공용으로 옮겨 음색표만 남았다. 기존 저장(progress·모드별 진행·골드·꾸미기)은 키 그대로라 이어진다. 자체 테스트 21/21 PASS.
- 2026-08-06: 아이폰 미니 화면 정리(사용자 신고 셋). 놀이 중 감춰뒀던 공용 상단 띠 복원(나가기·소리·설정 접근 복구 - 보드는 폭이 병목이라 세로 349px이 남아 손해 0) + 어두운 배경 위 프레임 요소(띠 아이콘·시작 화면 제목·안내·테두리 버튼)를 크림빛으로 뒤집음(팝업용 어두운 글자색을 물려받아 아이콘이 배경에 묻히던 문제). 보드+출구를 한 묶음으로 중앙 정렬(왼쪽 치우쳐 보이던 원인, 보드 270→296px) + 골드·이동 알약을 띠 가운데로. 좁은 화면 조작 버튼 두 줄(위=이전·다음, 아래=되돌리기·힌트·리셋 44px). 테스트 21/21 + 루트 172건 PASS, 4종 화면 실측 검증.
- 2026-09-06: 보드 안쪽 개편(B판). 보드 안쪽 수치를 전부 한 칸에서 파생(`--cell-px`/`--edge`/`--pad`/`--round`/`--drop`) - 테두리가 `3px` 고정이라 칸 대비 두께가 폰 가로 6.4% ~ 태블릿 2.6%로 2.5배 벌어져 있었다(04 컨벤션 3.3을 이미 어기던 자리). 블록을 카드 한 장으로(캐릭터가 여백만큼 물러나 밥풀이 가로3이 "세 마리"가 아니라 "한 조각"으로 읽힌다). 주인공은 배경을 채우는 대신 붉은 후광 + 진한 그림자로 띄웠다(테두리를 꺼도 남는다). **출구 화살표를 집으로 바꾸고**(사양도 코드 주석도 처음부터 집이었다) 출구 길을 행 전체로 넓혔다. 클리어 도착점을 집 앞으로 정정(옛 값은 3.5칸을 가서 지나쳤다). 평상 표정 풀 `calmFaces` 신설 - 화난·우는 컷이 평상에 나와 이야기와 반대였다. 사양 0.3의 동물 종류 넷·토끼는 그림 시트 전환(2026-07-02) 때 코드에서만 사라진 것이라 문서를 코드에 맞춰 정정. 자체 21/21 + 루트 268건 PASS, 네 화면 × 테마 5 × 꾸미기 4조합 × 스타일 2 실측.
