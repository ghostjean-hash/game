...
| `tiled: true` (1칸 조립) | 말랑이(`c`) | 1칸 정사각 캐릭터를 블록 길이만큼 반복 배치한다. 2칸=2마리, 3칸=3마리. 표정 그리드 방식(`faceSheet`)으로 그린다(아래) |
