...
모드는 **진행 맵(🗺) 안의 탭**으로 고른다(상단바에서 즉시 전환하지 않는다). 맵을 열면 현재 모드 탭이 선택돼 있고, 다른 모드 탭을 누르면 그 모드의 진행을 미리 볼 수 있다(`mapViewMode`). 실제 전환은 그 모드의 퍼즐을 고를 때 확정된다(`mapViewMode` → `activeMode`). 모든 팝업(상점/맵/설정/결과)은 라이트 테마다(style.css `.rushhour .modal`에서 색 토큰을 밝은 값으로 재정의 → 내부 요소가 상속).
