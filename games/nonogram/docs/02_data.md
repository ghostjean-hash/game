...
튜토리얼 3(5×5) + 초급 100(5×5) + 중급 100(10×10) + 고급 5(15×15). 고급은 중급까지 `LARGE_UNLOCK_CLEARS`개 클리어 시 해금. 실제 목록은 `puzzles.js` 배열이 SSOT이며, 전부 §3.2 검증 통과분만 포함한다(테스트가 전수 확인).

초급·중급 다수는 손으로 그린 그림(하트·고양이 등 앞쪽)이 아니라 **랜덤 좌우대칭 모양을 solver로 유일해·무추측 검증해 채택한 생성 퍼즐**이다(같은 방식으로 200종을 채웠다). 생성 재현 스크립트는 세션 스크래치에 있고, 산출물(puzzles.js)이 SSOT다. 색은 검증에 무관하므로 통과 모양 위에 파스텔 팔레트로 입힌다.

이름(title): 모든 퍼즐은 도감 수집용 고유 이름을 가진다. 수제 그림은 그림에 맞는 이름(하트·고양이 등), 생성 퍼즐은 이름 풀(꽃·과일·동물·간식·보석·하늘·사물)에서 고유하게 배정한다. 생성 퍼즐은 특정 사물 그림이 아니라 대칭 무늬이므로 이름과 모양이 반드시 일치하지는 않는다(수집용 이름). 번호 이름("초급 N")은 쓰지 않는다.
