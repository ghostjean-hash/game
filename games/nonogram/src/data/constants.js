...
export const CELL_FIT = {
  MIN_PX: 12,                        // 전체 맞춤 계산의 바닥값(= 두 손가락 축소 하한의 바닥).
                                     // 손가락 크기 보장은 ZOOM이 맡으므로 여기는 낮게 둔다.
...
// 두 손가락 확대·이동(docs/01_spec.md §3, docs/06_plan_zoom_pan.md).
// 좁은 화면(휴대폰)에서 큰 판을 한 화면에 다 넣으면 칸이 손가락보다 작아진다.
// 그때는 전체 넣기를 포기하고 칸을 키운 뒤 확대·이동에 맡긴다.
export const ZOOM = {
  FIT_MIN_PX: 26,   // 전체를 넣었을 때 이 크기 이상이면 그대로, 미만이면 확대·이동 모드
  START_PX: 34,     // 확대·이동 모드 진입 시 기본 셀 크기(손가락으로 누를 수 있는 크기)
  MAX_PX: 64,       // 확대 상한
  CANCEL_MS: 300,   // 두 손가락이 닿기 직전 시작된 칠하기를 오조작으로 보고 되돌리는 시간 한계
};
