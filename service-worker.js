...
// v177 (2026-07-10): flightshooting 유도탄·메인 빔 크기 절반 + HUD 강화 표기 개편(별 제거 → 로마숫자 티어 + 아라비아 서브, 마스터 시 주황색). 유도탄 HUD 로마 범위 초과 버그(11→X) 수정. PWA 옛 캐시 무효화.
// v178 (2026-07-10): flightshooting 발열 최적화(총알 3종 글로우 제거 - 다수 요소 발열 방어, 단일 요소만 글로우) + 무기 문서 전면 정합(docs/05 재작성·01·02·07 발별 순차 사양으로, numbers 주석 정정). PWA 옛 캐시 무효화.
// v179 (2026-07-10): flightshooting 자동 플레이 AI 몬테카를로 강화 - 예측 지평 확대, 위쪽 빈 공간 후보·무작위 도피처 추가, 여유공간 도피(위험할 때만). 회피·위치 개선. PWA 옛 캐시 무효화.
// v180 (2026-07-12): flightshooting 원격 조정 4건 - 유도탄 0강화 크기↑·메인 총알 단계색 대비↑(명도 교차)·보스 등장 왼쪽 점프 제거·어린이 모드 방사탄 감축(radialMul). PWA 옛 캐시 무효화.
// v181 (2026-07-12): flightshooting 원격 2차 - 어린이 목숨 5개(HUD 하트 5칸)·메인 총알 속도 강화 무관 고정·메인 총알 외형 전면 재설계(색만 바뀌는 단계 제거 + 빔 폭 레인 고정으로 옆칸 침범 제거). PWA 옛 캐시 무효화.
// v182 (2026-07-12): flightshooting 원격 3차 - 무기 외형 다듬기(사이드 크기 tier10 2/3 축소·메인 총알 단계별 다른 무늬 패턴 복원·유도탄 삼각→화살표 형태 + 색 11색화). PWA 옛 캐시 무효화.
...
// v205 (2026-08-01): 시작 흐름 공용 프레임 2단계 - 노노그램에 실제 적용. 시작 화면이 새로 생기고, 스테이지 지도가 시작 다음 칸으로 내려가며, 결과가 전용 화면에서 플레이를 덮는 카드로 바뀐다. 되돌아가기는 화살표·기기 뒤로가기 모두 한 칸씩만 올라간다. 옛 v204 캐시 폐기.
// v206 (2026-08-01): 구글 로그인 팝업 2번 + 1시간마다 풀림 수정. 조용한 갱신에서 팝업을 완전히 없애고(구글 부품은 prompt를 비워도 창을 열어 팝업 차단에 막혔다), 같은 순간의 중복 요청을 하나로 묶고, 만료된 로그인은 허브 진입 때 주소 이동으로 되살린다(콘솔에 리디렉션 주소 등록 전이라 스위치는 꺼둠). 옛 v205 캐시 폐기.
// v207 (2026-08-01): 시작 흐름 공용 프레임 3단계 - 나머지 네 게임(테트리스·비행 슈팅·러시아워·스도쿠)에 적용. 다섯 게임 모두 시작 화면이 앞에 서고, 되돌아가기가 한 칸씩만 올라가며, 결과가 덮는 카드로 통일된다. 러시아워·스도쿠는 시작 화면이 새로 생기고 스도쿠에는 소리 버튼과 이어서 하기도 새로 생긴다. 테트리스는 주소를 바꿔 페이지를 다시 읽던 모드 선택이 고르는 칸 하나로 접혔다. 옛 v206 캐시 폐기.
// v208 (2026-08-06): 아이폰 미니 손 조작 수정 둘. 노노그램은 큰 판(10·15칸)에서 칸이 손가락보다 작아 플레이가 불가능했다 - 두 손가락으로 확대·축소·밀기를 넣고, 오므리기 한 번이 전체 보기가 되게 했다(칸 15칸 18→34px, 10칸 20→27px). 러시아워는 놀이 중 사라졌던 나가기·소리·설정 버튼을 되살리고, 어두운 배경에 묻히던 아이콘·글자를 크림빛으로 바꾸고, 보드+출구를 한 묶음으로 중앙에 놓고(보드 270→296px), 하단 조작 버튼 다섯을 두 줄로 나눠 손가락 크기로 키웠다. 옛 v207 캐시 폐기.
// v209 (2026-08-06): 영어 단어장 "다시 안 보기"를 두 갈래로 나눴다. 이미 아는 기초 단어는 학습 대상에서 아예 빠지고(완료율 계산에서도 빠진다), 외운 단어를 내리는 자리는 복습 목록에서만 빼서 외운 개수와 완료율이 그대로 남는다. 외운 단어 목록에서 단어마다 또는 한 번에 넘길 수 있고, "다시 안 볼 단어" 화면이 두 묶음으로 나뉜다. 옛 v208 캐시 폐기.
// v210 (2026-08-06): 위 두 갈래의 이름을 "1차/2차"에서 뜻이 드러나는 말로 바꿨다 - "이미 아는 단어"와 "완전히 외운 단어". 순서처럼 읽혀 무엇을 뜻하는지 알 수 없다는 지적을 받았다. 학습 화면 버튼은 "이미 아는 단어로 빼기", 외운 단어 목록 버튼은 "완전히 외움"이다. 옛 v209 캐시 폐기.
// v211 (2026-08-06): 영어 단어장에 아카이브를 만들었다. 다시 안 볼 단어는 이제 그 세트에 없는 것과 같아서, 200단어 중 197개를 빼면 그 세트는 3단어짜리가 된다 - 목록·남은 개수·완료율 어디에도 뺀 수가 나오지 않고 홈의 아카이브에서만 본다. 세트를 누르면 한 번 더 뜨던 진도 화면을 없애 바로 학습으로 들어가고, 복습은 학습 화면 위쪽 버튼으로 옮겼다. 옛 v210 캐시 폐기.
...
// v240: 오늘운동 관리 화면 아홉 + 달력 신설. 앱 안에서 운동 템플릿 → 루틴 → 요일 규칙 →
//        월간 계획 생성까지 만들 수 있게 되어 오늘 화면에 실제 운동이 뜬다. 옛 v239 캐시 폐기.
// v241: 오늘운동 월간 계획 생성이 지난 날짜를 만들지 않는다. 이번 달은 오늘부터 월말까지,
//        지난 달은 만들기 자체가 없다. 옛 캐시가 남으면 옛 생성 규칙이 그대로 돈다.
// v242: 허브 대화상자가 검은 바탕에 검은 글자로 떠 읽히지 않던 것 정정. 로그인 충돌 창이
//        그 자리였다. 옛 CSS 가 캐시에 남으면 글자가 계속 안 보인다.
// v243: 허브가 공용 색 토큰 자체를 밝은 값으로 다시 정의한다. v242 는 대화상자 한 부품만
//        덧칠한 것이라 다른 공용 부품에 같은 사고가 남아 있었다.
...
const PRECACHE = [
  ...
  "./shared/frame/wallet.js",
  "./shared/frame/shop.js",
  "./shared/frame/shopcard.js",
