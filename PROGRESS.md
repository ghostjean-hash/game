...
27.4. 미해결: (a) v0.4 기획서 사용자 확정 대기 - 확정 시 스포트라이트 효과 지문 제작 → 구현 → 검증 → 배포 (b) 앱 코드는 아직 v0.3 문법 스캔 상태 (c) web-deploy smoke의 CDN 전파 감지 보강 후보는 글로벌 buffer 인계. web-deploy 도입과 SW 경로 전역 결함 수정은 26장 참조.
