...
> 22차 정리: 2026-05-19. Sprint 090(절 2.104, 백캐스트 + 자동 history 등록 폐기 + "내 번호로 선택" 진입점 + 후속 12건) archive 추가 이전. Sprint 097 신설(기록 탭 reveal 게임화 - 발표 대기 ? 7개 + 발표 후 ball 마스킹 + 체크 클릭 순차 reveal + 일치 바운스)로 활성 8건 도달 → 룰 1.6 자동 적용. Sprint 091~097 활성 잔존.
>
...
## 2.104. Sprint 090 완료 - 백캐스트 + 자동 history 등록 폐기, "내 번호로 선택" 진입점 (S090, 2026-05-17, archive 22차 이전 2026-05-19)

배경: 사용자 명시 3건 연속 + 후속 7건 (절 2.104.11~2.104.17).
1. "전적이 테스트용이야?" → 백캐스트 정직성 의문.
2. "진짜를 돌리고 싶어." → 진짜 누적 의향.
3. "직접 선택" → 자동 등록 폐기.

핵심 결정: 백캐스트 전면 폐기 + 자동 history 등록 폐기 + saved-sets-row "내 번호로 선택" → "확정" (후속 단축) + 회차당 cap 5 (후속 7에서 무제한) + 옛 데이터 자동 마이그레이션 + 어휘 "확정"/"취소".

**변경 핵심**:
- `core/history.js`: backfillRecommendations 폐기, toggleSavedSetRegistration / countRegisteredForRound / isRegistered 신설, source: 'user' 필드 추가
- `data/numbers.js`: HISTORY_REGISTER_CAP_PER_ROUND 신설(후속 7에서 dead), BACKFILL_RECENT_COUNT dead(S093 폐기)
- `data/storage.js`: S090 마이그레이션 - 백캐스트 추정 항목 createdAt 매칭 자동 제거
- `render/main.js`: 자동 호출 제거, "확정" 토글 핸들러
- `render/saved-sets-section.js`: row에 "확정" 버튼 + 카운터
- `styles/main.css`: 확정 row outline / 좌측 바 / background tint / cap hint (후속에서 진화)
- docs SSOT: 01_spec 5.2.5.9 / 02_data 1.16 / 3.7

**후속 12건 (2.104.11~2.104.17)**:
- 후속 1 (2.104.11): 라벨 단축 "내 번호로 선택" → "확정"
- 후속 2 (2.104.12): 전적 초기화 버튼 + 현재 회차 발표 대기 섹션 신설
- 후속 3 (2.104.13): 발표 대기 회차 라벨 중복 폐기
- 후속 4 (2.104.14): UI 정렬 + 1회 강제 클린업 (lotto_s090_cleared flag)
- 후속 5 (2.104.15): 확정 row 시각 (outline → inset bar)
- 후속 6 (2.104.16): row padding 정합 + 정렬 일치 (자비스 14건째 결손 자인)
- 후속 7 (2.104.17): UX 종합 6건 - 세로 띠 폐기 + 뒤쪽 여백 절반 + 배경 강화 + 휴지통 결합 + cap 무제한

**검증**: node tests/run-node.js → 315/315 PASS (후속 4에서 316으로 증가).

**전체 본문은 본 archive 절 자체에 흡수**. 본문 토큰 부담으로 압축 표기 (활성 7건 cap 정합 위해 archive 이전).
