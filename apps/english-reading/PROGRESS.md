...
1.3. **마지막 갱신**: 2026-08-13 (지문 200편 전량 완성 - 2.54).

## 1.4. 어려운 문장 수집 (2026-07-24)

- 문장 끝의 기존 복사 아이콘을 점 3개 메뉴로 전환. 메뉴는 `복사`와 `어려운 문장 저장` 두 항목을 제공하며, 저장된 문장은 같은 메뉴에서 해제 가능.
- 상단 `어려운 문장` 화면에서 저장 문장·출처를 모아 보고, 문장을 누르면 원래 지문과 해당 문장 위치로 돌아가 다시 학습.
- 저장 데이터는 `localStorage`의 `savedSentences`에 문장 원문·지문 출처·문장 순서로 기기별 보관. standalone도 재생성.
- 단어장·어려운 문장 카드는 뜻/해석을 아래로 펼치지 않고 `보기` 바의 글자를 직접 뜻·해석으로 교체하는 토글로 변경. 어려운 문장 카드의 출처 제목은 제거.
- 토글 바는 보기 문구와 실제 뜻·해석의 최대 높이를 항상 확보해, 토글해도 아래 카드와 글줄의 위치가 움직이지 않게 보정.
- 지문 목록·읽기 상단 제목을 영어 title 중심으로 전환. 읽기 상단의 영어 제목을 누르면 한글 제목을 3초간 보여 준 뒤 자동 복귀.
...
## 2.48. 난이도 우선 개편 - 코스 축을 주제→난이도로 전환 (2026-07-21, 2.47 후속)

직전 세션에서 사용자가 "주제를 고르면 그 안에서 1~3단계가 순차로 나오는" 구조를 이상하다고 지적했고("보통 난이도라 하면 1단계를 고르면 1단계에 다양한 주제가 나오는 방식"), 이 세션 첫 지시 "난이도 우선 방식으로 변경해"로 실제 전환에 들어갔다.

- 설계 결정(자비스 판단, surgical 우선): 데이터(passages.json)의 주제별 저장(courses: daily-life/relationships)은 콘텐츠 원본·출제 힌트용으로 그대로 두고, 앱 표시 계층에서만 난이도(level)별로 재그룹핑한다. 데이터 구조를 갈아엎으면 출제·검증(authoring-index·validate·lint) 파이프라인 전체가 흔들려, 표시 로직만 바꾸는 쪽을 택했다.
- core/course.js: createLevelCourses(courseList, labels) 신설 - 모든 코스의 지문을 level별 "레벨 코스"로 재편(레벨 안은 주제 등장 순서로 묶고 같은 주제는 원래 순서 유지, 안정 정렬). 기존 createCourse/courseProgress는 유지(courseProgress는 레벨 코스에도 그대로 재사용). 현 40편 분포 = Lv1 18편·Lv2 18편·Lv3 4편.
- main.js: rebuildCourse가 createLevelCourses로 코스 생성(LEVEL_LABELS = Level 1 입문/2 기초/3 중급). 홈 요약 "코스 N개"→"난이도 N단계", 레벨 카드 부제에 섞인 주제 표시(Daily Life · Relationships). 지문 목록 카드의 Lv 배지→주제(topic) 배지로 교체(같은 레벨이라 난이도 표시 무의미). 클리어=한 레벨 전체 완주로 자동 전환.
- style.css: .lv 배지에 white-space:nowrap 추가(주제명이 길어도 pill 안 깨지게).
- 문서: CLAUDE.md 1.3·3.4를 난이도 우선으로 갱신(3.4 "코스=난이도 레벨", 데이터는 주제별 저장 유지+표시만 재편 명기).
- 검증: tests/run-node.mjs 전체 통과(createLevelCourses 유닛 6건 추가 - level 오름차순·주제 취합·주제별 묶음 정렬·labels·courseProgress 재사용). browser-shot 로컬 4종(홈 난이도 3단계 / Level 1 목록 Daily Life 배지 / Level 1 마지막 Relationships 배지 / 지문 진입+전체해석 채점) + 배포 URL 최종 확인, 모두 콘솔 0.
- 배포: /web-deploy 완주(commit acfae9f, smoke 2 URL 200·콘솔 0), SW 캐시 v189→v190 bump. 배포 URL이 처음엔 CDN 전파 지연으로 옛 화면(코스 2개)을 서빙했으나 약 90초 후 난이도 3단계 화면으로 전파 확인. standalone 재빌드(360KB).
- 잔여: 다음은 커리큘럼 41번(Travel) 이후 지문 추가. 숙어 배정 대조 lint(커리큘럼 숙어표 데이터화 선행)는 미구현 유지.

## 2.49. 지문 40편 추가 - Relationships 완성 + Travel·Health 코스 신설 (2026-07-21, 2.48 후속)

난이도 우선 개편(2.48) 직후 사용자 "40문제 추가" 지시. 착수 전 현 상태를 커리큘럼 지도(docs/authoring/CURRICULUM_REVIEW.md 200편 확정본)와 대조하니 Relationships가 지도상 18편 설계 중 12편(통번호 29~40)만 차 있었다. 그래서 "다음 40편"을 커리큘럼 통번호 41~80으로 제안(관계 마무리 6 + 여행 18 + 건강 16)했고 사용자가 A안으로 확정했다.

- 범위 확정(사용자 A): 통번호 41~80. Relationships 41~46(Lv2 2 + Lv3 4)으로 코스 완성(12→18편), Travel 47~64(Lv1 7·Lv2 7·Lv3 4 = 18편) 신설, Health 65~80(Lv1 5·Lv2 7·Lv3 4 = 16편) 신설.
- 출제 방식(2.47 draft-only 패턴 확장): 40편을 주제·난이도 경계로 11개 배치로 쪼개 general-purpose 에이전트 11개 병렬 위임. 공용 브리프(_BRIEF.md - 규칙·스키마·샘플 2개 경로·배치 규칙·기존 id 40개)를 각자 Read하고, scratchpad 임시 파일에만 Write + 자체 validate-draft로 0에러까지 검증. passages.json은 아무도 안 건드려 동시 조작 사고 원천 차단. 11배치 전부 자체 검증 통과 보고.
- 취합·반영은 자비스 직접: 11개 draft를 _merge.mjs로 합쳐 편수(6/18/16)·id중복 0 확인 → 40편 합본을 validate-draft로 전량 재검증(형식·기존대조·draft내 중복) 40/0 통과 → --apply로 relationships 코스에 6편 push + travel·health 코스 신설(courses 배열 4개로). run-node strict 전체 통과, 새 40편 lint 경고 0(기존 daily-life 12건만 잔존).
- 코스 구조: passages.json courses = [daily-life 28, relationships 18, travel 18, health 16] = 80편. 난이도 우선 표시라 홈은 여전히 "난이도 3단계"(Lv1 30·Lv2 34·Lv3 16)이고 각 레벨에 4개 주제가 섞여 나온다(createLevelCourses가 재그룹핑).
...
## 2.51. 지문 81~120번 40편 전면 보강 + 목록 스크롤 리스너 누수 수정 (2026-07-28)

사용자 지시는 2단계였다. 먼저 "81~120번까지 codex로 작업했는데 이상이 없는지 전수 검사해줘", 검사 결과를 받은 뒤 "모두 수정해. 문제의 수준이 떨어지는 부분이 있으면 다 교체해도 되니 보강해줘". 2.50에서 봉합한 40편이 자동 형식 검증만 통과했을 뿐 기존 80편 수준에 못 미친다는 것이 전수 검사로 드러났고, 그 격차를 메우는 재작업까지 한 cycle에서 끝냈다.

- 전수 검사 방식: 40편 200문장을 5편씩 8배치로 갈라 general-purpose 에이전트 8개 병렬 감수(항목 13종 - 영어 자연스러움·서사 논리·커리큘럼 정합·난이도·직독직해·완역·단어 뜻·문법 이름표·핵심 어순·심화 카드·끊기 규칙·제목·중복). 감수 결과는 자비스가 원본 대조로 재확인했고, 기계로 셀 수 있는 항목(숙어 반영·필드 밀도·wordOrderPoint와 grammar 라벨 동일·words 중복)은 기존 80편과 수치 대조해 판정 근거로 삼았다.
- 검사가 밝힌 것: (a) breakRules가 200문장 **전부** 빈 배열이라 5등급 채점 중 허용·비추천 두 등급과 이유 카드가 신규 지문에서 미발동(기존 80편은 400문장 중 350문장 87.5% 충전). (b) insight 0편(기존 14편). (c) chunks가 199/200문장에서 정확히 2덩어리 고정이라 동사-목적어·사역보어를 가르는 끊기가 대표 정답으로 등록됨(기존은 2~4덩어리). (d) 커리큘럼 지정 숙어 9편 누락(catch up·go over·stick to·get back to·figure out·break down·stick with·turn into·step out of - 제목에만 존재). (e) 틀린 문법 설명 2건(86 목적격 관계대명사를 주격이라 함, 93 When절과 주절 역할 뒤집음). (f) 소재 어긋남 5편, 시제 혼재 1편, 어색한 영어 다수, words 중복 1건.
- 보강 방식(2.49/2.50 draft-only 패턴 유지): 공용 브리프(FIX_BRIEF.md - 결함 11종·불변 조건·모범 샘플 2편 경로·자체 검증 절차) + 배치별 개별 지적을 8개 에이전트에 병렬 위임. 각자 scratchpad에만 Write하고 passages.json은 아무도 안 건드려 동시 조작을 차단했다. id·level·topic은 불변(학습 기록 단절 방지), title·titleKr·sentences는 전면 교체 허용.
- 검증기 한계 처리: tools/validate-draft.mjs는 현재 passages.json 전체와 대조하므로, 같은 id를 교체하는 이번 작업에서는 자기 자신과 id 중복이 나 항상 FAIL한다. 자비스가 '교체 후 상태'(같은 id 원본을 대조 대상에서 제외)로 돌리는 하니스(check-out.mjs)를 만들어 40편 전량을 재검증했다.
- 결과 수치: breakRules 0 → 678항목(200/200 문장 100%, 기존 80편 442항목보다 촘촘). insight 0 → 37문장(26편, Lv3는 편당 2개). chunks 문장당 평균 2.00 → 2.76. grammar 1.00 → 2.12, words 0.92 → 1.25. 커리큘럼 숙어 18/18 전부 본문 반영(미반영 0). lint 경고는 120편 16건 → 12건으로 줄어 신규 40편 발 경고가 0이 됐다.
- 코드 결함 3건(같은 세션에서 자비스 직접 수정): (1) 목록에 붙인 스크롤 저장 리스너가 화면을 떠나도 해제되지 않아, 단어장·홈으로 나가면 stage가 비면서 scrollTop 0이 목록 위치로 저장되고(복귀 시 맨 위로 튐) 홈에서는 course가 null이라 `TypeError: Cannot read properties of null (reading 'id')` 콘솔 에러가 났다. 화면 전환 공통 지점인 setTop()에서 반드시 해제하도록 고쳤다. (2) 스크롤 복원을 rAF 1회로 처리해 레이아웃 확정 전 값에 잘릴 수 있던 것을 다중 프레임 재시도로 바꾸고, 복원 중간값이 저장을 덮지 않게 리스너를 복원 완료 후 붙인다. (3) 죽은 코드 제거 - style.css `.passage-card.done-perfect`·`.passage-card .lv`(요소 자체가 사라짐), main.js getDoneMeta/computeChunkOk/passageHasVocab + doneMeta 저장(2.50에서 목록 표시가 없어져 쓰기 전용 상태가 됨).
- 검증: run-node 120편 strict 전량 통과. playwright 실경로 재생 2종 - 스크롤 왕복 4경로(지문·단어장·홈·문장목록) 전부 900 정확 복원 + 콘솔 에러 0(데스크톱·모바일), 보강한 Lv3 지문에서 5등급 채점 5종(추천·허용·비추천·다른분할·놓침)이 한 문장에 동시 발동 + 비추천 이유 카드·직독직해 3덩어리·자연스러운 해석·핵심 어순·심화 카드 4필드 전부 표시, 콘솔 0. standalone 재빌드(909→1091KB).
- 주의(다음 출제 때): validate-draft가 통과해도 품질을 보장하지 못한다. chunkViolations 4규칙은 '동사와 목적어를 가르는 2등분'을 잡지 못하고, breakRules·insight는 비어 있어도 통과한다. 출제 시 이 세 가지는 사람(또는 감수 에이전트)이 따로 봐야 한다.
- 잔여: 커리큘럼 121번 이후 지문. 숙어 배정 대조 lint(커리큘럼 숙어표 데이터화 선행)는 미구현 유지 - 이번엔 일회용 스크립트로 대조했다.

## 2.52. 기존 지문 1~40번 보강 + 끊기 이유 오표시·검증 구멍 수정 (2026-07-28)

사용자 지시 "1~80번도 검사해줘" → "고칠게 있으면 수정해줘" → "나머지 배치 끝나면 병합하고 배포까지". 작업 도중 토큰 소모 지적을 받아 41~80편은 중단했고, 완료된 1~40편만 반영했다.

- 검사: 1~80편 80편을 5편씩 16배치로 나눠 감수 8배치(1~40편)를 돌렸다. 그중 2배치(01·06)는 API 타임아웃으로 실패해 산출물이 없었다. 41~80편은 감수 없이 기계 진단(배치별 결함 수치 파일)만 만들어 보강 단계에 넘겼다.
- 기계 대조로 확인: 커리큘럼 배정 숙어 44개는 1~80편에 전부 반영돼 있었고(미반영 0) 단어 중복도 0이었다. 반면 끊기 규칙은 지문당 평균 5.5항목(400문장 중 50문장 공백, allowed 거의 0), 심화 카드는 Lv3 16편에 각 1개, 400문장 중 264문장이 2덩어리 고정, 핵심 어순과 문법 카드가 같은 항목을 반복하는 문장이 74개였다.
- 코드 결함 2건 수정: (1) `chunking.js:boundaryReason`이 조각 첫 낱말이 that이면 무조건 "that절 앞"을 표시해 `that morning` 같은 지시형용사에서 틀린 이유를 노출했다(시간 명사 화이트리스트 예외 추가). (2) `validate.js`가 `discouraged`만 대표 경계와 대조하고 `allowed`는 안 봐서, 대표 경계와 겹친 allowed가 화면에서 영구히 발동하지 않는 죽은 데이터로 남았다(규칙 추가 + 기존 2건 교정).
- 보강 1~40편: 8배치 병렬(draft-only). 끊기 규칙 710항목(200문장 100%, 기존 대비 3배), 심화 카드 26개, 끊기 조각 평균 2.62덩어리, 카드 중복 0. 레벨 이탈(Lv1의 사역·준사역·to부정사·that절, Lv2의 과거완료), 어색한 영어(a small talk·The sound rings·felt strange and lonely 등), 제목과 본문 불일치(새 세탁기 없음·이사 언급 없음·clear out 숙어 선점), 지시 대상 혼재(closet↔shelf·kitchen↔room), 번역 카드 불일치를 함께 교정했다. **정성 lint 경고가 120편 12건에서 0건이 됐다.**
- 검증: run-node 120편 strict 통과, 숙어 62개(1~80편 44 + 81~120편 18) 전수 반영 확인, playwright 실경로 2종(보강 지문·기존 보강본 각 1편에서 5등급 채점 5종 동시 발동 + 이유 카드 위치 교정 확인, 콘솔 0). standalone 1231KB, SW v193→v194.
- 미완: **41~80편 40편은 보강하지 않았다**(중단). 기존 상태 그대로이며 위 결함이 남아 있다. 재개 시 감수 단계 없이 보강 1단계로 진행하고, 배치를 5편이 아니라 10편 단위로 묶어 에이전트 수를 절반으로 줄일 것(고정비 중복이 이번 토큰 과소비의 주 원인).
- 이번 cycle의 자기 반성: 감수와 보강을 2단계로 분리해 같은 지문을 두 번 읽혔고, 80편을 16배치로 잘게 쪼개 규칙 파일 재독 고정비를 16번 물었다. 41~80편이 감수 없이도 보강에 성공한 것이 1단계로 충분했다는 증거다. 또 "검증 통과"를 보고할 때 그것이 형식 검증에 한정된다는 점을 분명히 하지 않았다 - 화면 실경로 확인은 매번 표본 1~2편뿐이었다.

## 2.53. 121~200번 80편 출제 준비 - 배치 계획·공용 브리프·모범 샘플 고정 (2026-07-30)

사용자 질의("영어 지문 어디까지 만들었지")로 현황을 실측한 뒤, 사용자가 "2순위 진행해. 빠르게 진행한 후 보강하는 방식으로 가자"로 신규 출제를 택했다. 배치 발사 직전에 사용자 지시("토큰 98% 소모 시점에 멈추고 /jc")로 봉합해 이번 cycle은 준비물까지만 남긴다.

- 실측 현황: 120편 600문장(Lv1 43 / Lv2 50 / Lv3 27). 주제로는 Daily Life·Relationships·Travel·Health·Study·Self Development가 계획 수량을 채웠고 Language만 8/16이다. Technology·Psychology·Games·Science·Critical Thinking 72편은 미착수. 남은 80편이 곧 커리큘럼 121~200번이고 그 레벨 구성(Lv1 17 / Lv2 30 / Lv3 33)이 목표 분포 부족분과 정확히 일치한다.
- 사용자 결정 반영: 감수 단계를 따로 두지 않는 1패스 출제로 간다. 품질 보강은 출제 완료 후 별도 단계로 미룬다.
- 산출물 3종(모두 신규, 코드·데이터 변경 0): `docs/authoring/AUTHOR_BRIEF_121-200.md`(배치 에이전트 주입용 공용 브리프) / `docs/authoring/BATCH_PLAN_121-200.md`(10배치 분할 + 취합·검증·배포 순서) / `docs/authoring/samples/` 3편(2026-07-28 보강을 거친 Lv1·Lv2·Lv3 지문 사본, 완성 기대치 기준).
- 브리프에 명문화한 품질선은 CLAUDE.md 4.6이 지목한 '검증 통과가 품질을 보장하지 못하는 3종'을 정면으로 겨눈다 - chunks 동사·목적어 2등분 금지(예시 3개 명시)와 문장당 2~4덩어리·지문 평균 2.6덩어리, breakRules 문장당 2개 이상·지문 15개 이상, insight Lv3 편당 1~2개·Lv2 0~1개·Lv1 0개, grammar label에 어휘·연어 금지. 여기에 기존 120편이 이미 쓴 숙어 57개 목록을 넣어 재사용을 피하게 했고, 레벨별 낱말 수·길이 리듬·시작어 반복·레벨 초과 문법은 lint 경고 0을 목표로 지시했다.
- 배치 크기를 5편에서 7~10편으로 키웠다. 2026-07-28 자기 반성(5편씩 16배치가 규칙 파일 재독 고정비를 16번 물었다)의 직접 반영이다. 배치는 주제 경계를 넘지 않게 잘라 배치 안에서 소재·시간대 분산을 스스로 관리하게 했다.
- 검증: 문서 3종은 즉시 동작이 없어 동작 검증 불가 - 참조 일치만 확인했다(브리프의 수치·규칙이 `core/validate.js` lint 범위, `chunking.js:chunkViolations` 4규칙, 앱 CLAUDE.md 4.3·4.6과 어긋나지 않는다). 샘플 3편은 `passages.json`에서 스크립트로 떠 온 사본이고 파일 존재·크기(9.7 / 11.4 / 12.9KB)를 확인했다. 현황 수치는 전부 `passages.json` 실측 스크립트 출력이다.
- 미착수·잔여: 배치 10개 출제는 발사 전이다(draft 0건, `passages.json` 무변경). 41~80번 40편 품질 보강도 2026-07-28 중단분 그대로 남아 있다.

## 2.54. 121~200번 80편 출제 완료 - 200편 전량 달성 + 새 주제 5개 신설 (2026-08-13)

사용자 지시("121~200까지 병렬로 진행해줘")로 2.53에서 준비만 해 둔 10배치를 한 번에 발사했다. 배치마다 공용 브리프(`AUTHOR_BRIEF_121-200.md`)와 자기 구간 배정표만 주고 자기 draft 파일에만 쓰게 해 `passages.json` 동시 조작을 원천 차단했다.

- 산출: 80편 400문장. 새 코스 5개(Technology 16 / Psychology 16 / Games 14 / Science 16 / Critical Thinking 10)가 생기고 Language가 8편 더해져 16편으로 찼다. **총 200편 1000문장, 레벨 분포 Lv1 60 / Lv2 80 / Lv3 60으로 애초 목표 분포를 정확히 달성**했다.
- 배치별 품질은 브리프 기준선을 전부 넘겼다 - breakRules 1724항목(문장당 4.31, 빈 문장 0), insight 92개(Lv3 33편 전편 보유, Lv2 30편 중 26편, Lv1 0), chunks 문장당 2.77덩어리, grammar 문장당 2.40, words 570개. 2026-07-28 재작업 사유였던 '빈 breakRules·Lv3 insight 0'이 이번엔 0건이다.
- 취합 검사(자비스 직접): 배정표 대조 결과 80편 전수의 레벨·주제가 지도와 일치했고, id·제목·문장 중복이 기존 120편과도 배치 사이에서도 0이었다. 배정 숙어 34종은 전부 본문 등장 + `words` 등록을 확인했다(초기 3건이 미반영으로 잡혔으나 `came to mean`·`came up with`·`leaving out` 활용형이라 검사식 오탐이었다).
- 검증기가 못 잡는 항목은 따로 스캔했다. 동사·목적어를 가르는 끊기 후보 19건을 전수 열어 18건은 정당한 경계(절·전치사구·콤마 앞)로 판정하고, 실제 결함 1건(`She asks / about the arrow` - 동사와 딸린 전치사 분리)만 한 덩어리로 병합한 뒤 그 자리를 비추천으로 옮겨 이유 카드가 뜨게 했다. grammar label에 어휘를 적은 사례는 0건이었다(영문 label 9건은 전부 상관접속사·비교 구문).
- 자동 검증: 병합 후 `run-node.mjs` strict 200편 전량 통과, 정성 lint 경고 0건. `standalone.html` 재생성(2139KB), SW 캐시 v211→v212.
- 화면 실경로 검증(시스템 Chrome + playwright-core): 신규 지문 3편에서 끊기 5등급 채점(추천·비추천·다른분할·놓침 동시 발동)·비추천 이유 카드·직독직해와 자연 완역 분리·핵심 어순 기본 노출·문법 접기/펼치기·검토 후 선 잠금·해설 접힘 토글·전체 해석 후 완료 버튼 전환까지 확인했다. 단어 갈래는 환경설정에서 '단어 터치'를 켜는 실제 경로를 밟아 숙어 3토큰 묶음 수집(오렌지+토스트+뜻 감춤) → 해석 시 [단어-뜻] 리스트 공개 → 단어장 영구 저장 → 단어장 화면 표시까지 확인했다. 콘솔 에러 0.
- 이번에 알게 된 것: '단어 터치'는 기본 꺼짐이 설계값(끊기 틈과의 오터치를 막으려 사용자가 환경설정에서 켠다)이다. 검증 초반에 단어 클릭이 안 먹는 것을 결함으로 의심했으나 `pointer-events: none`이 그 설정의 정상 반영이었다.
- 잔여: 41~80번 40편 품질 보강은 2026-07-28 중단분 그대로다(끊기 규칙 문장당 1.26개로 보강분 3.4~3.6개에 못 미치고 심화 카드도 11개뿐).

## 2.55. 지문 120~200번 81편 내용 감수 289건 + 전량 수정 착수 (2026-08-13, 진행 중)

사용자가 "120~200번 내용 검토해줘"로 감수를 지시했고, 결과를 보고받은 뒤 "모두 수정해"로 전량 수정을 결정했다. 이 절은 감수 완료 + 수정 절반 진행 상태에서 봉합한 기록이다.

- **감수(완료, 커밋 a323ead)**: 81편 405문장을 감수자 10명에게 주제별로 나눠 독립 재검사시켰다. "형식 검증기를 통과했다는 사실을 품질 근거로 삼지 말라"고 명시해 내용만 다시 읽게 했다(앱 CLAUDE.md 4.6 + 글로벌 #428). 결함 **289건**(높음 35 / 중간 136 / 낮음 118). 결과는 `docs/authoring/REVIEW_120-200_2026-08-13.md`에 남겼다.
- 높음 35건의 성격은 사실·개념 오류 14(부력을 무게로 설명·보름 뒤 일주일을 초승달로·백신 통증을 효과 증거로·상관과 인과를 가르치며 스스로 과잉 추론), 문법·해설 오류 10(구동사 목적어 위치를 틀리게 규칙화·정상 문장을 비문 예시로·끊기 이유 위치 어긋남), 오역 6(look at the window를 '창밖을 보다'로·the same difficult stage를 '똑같이 어려운'으로), 영어·이야기 성립 5(원어민이 안 쓰는 결합·영어 제목과 한글 제목이 정반대·'네 낱말'이 실제로는 다섯 낱말)이다.
- **되풀이 유형 10종**을 따로 정리했다. 어순 카드가 문법 카드와 같은 말을 반복(60문장 이상), 끊기 이유 번호가 한 칸씩 어긋남(17건), 지문 사이 규칙 충돌(구동사 목적어 위치·look at 끊기·of 수일치·장소와 때 어순 등 5종), 제목-본문 불일치(8건 이상)가 핵심이다. 배치를 나눠 만들면 배치 안에서만 일관되고 사이에서는 어긋난다는 것이 이번에 드러난 구조적 한계다.
- **수정(진행 중)**: 공용 수정 지침(`FIX_GUIDE.md`)에 지문 사이 통일 기준 8종을 명문화하고, 기존 지문을 같은 id로 교체하는 작업에서 `validate-draft.mjs`가 자기 자신과 id 중복으로 항상 실패하는 문제를 피하려 전용 검사기(`check-fix.mjs`)를 따로 만들었다. 봉합 시점까지 10묶음 중 6묶음(Language 9편 32건 / Technology 16편 58건 / Psychology 16편 54건 / Games 7편 24건)이 끝났고 전부 형식 검사 FAIL 0·경고 0이다. 나머지 4묶음(Games 7편 / Science 16편 / Critical Thinking 10편)은 돌고 있다.
- 수정 담당들이 감수가 놓친 것을 추가로 잡았다 - 끊기 번호 오프바이원 2건(psychology-2), 낱말을 인용부호로 감싸면 토큰 분리기가 아포스트로피를 낱말의 일부로 봐 그 단어가 화면에서 안 눌리는 제약(language). 후자는 지침에 반영해 뒤 묶음이 같은 실수를 하지 않게 했다.
- **잔여**: 수정 5묶음 완료 대기 → 수정본 10개를 `passages.json`에 병합 → 200편 전량 검증 → 화면 확인 → 배포. 병합 전이라 앱에 반영된 것은 아직 없다(배포된 지문은 감수 전 상태 그대로).

### 2.55 후속 (같은 날 두 번째 봉합)

- 수정 완료가 7묶음으로 늘었다(Technology 16편 58건 / Language 9편 32건 / Psychology 16편 54건 / Games 7편 24건 / Science 8편 26건). 전부 형식 검사 FAIL 0·경고 0.
- 수정 담당들이 감수 목록 밖에서 추가로 잡은 것 - 끊기 번호 밀림(psychology-2 2건, science-1의 rainbow·windowsill 자기모순 2건), 라벨 오분류 2건(빈도 부사가 아닌 것을 빈도로, 현재형을 동사원형으로), 분리 가능 구동사 통일을 한 파일 안에서 두 곳 동시 적용(games-2의 turn around + put down).
- **작업 사본 위치를 `apps/english-reading/fixes/`로 옮겨 저장소에 커밋했다.** 세션 임시 폴더에 두면 세션이 끝날 때 사라져 7묶음 수정 결과가 날아간다. 같은 폴더에 전용 검사기(`check-fix.mjs`)와 공용 수정 지침(`FIX_GUIDE.md`)도 함께 둔다.
- 남은 3묶음(Games 7편 · Science 8편 · Critical Thinking 10편)은 권한 창 반복 때문에 중단시켰다. 중단 시점의 부분 수정 상태가 그 폴더 파일에 남아 있어, 재개 시 이미 고쳐진 부분을 확인하고 남은 것만 처리하면 된다.

### 2.55 완료 (2026-08-18)

- 보존한 10개 수정본을 전수 재검증했다. 총 81편 모두 `check-fix.mjs`에서 FAIL 0·lint 경고 0이며, id 중복·누락도 없었다. 이전 절에 '중단'으로 남은 Games 후반·Science 후반은 실제 수정본이 이미 작성된 상태임을 확인했고, Critical Thinking에서 미반영이던 그래프 축·상관관계 지문의 사실 오류를 추가 교정했다.
- `the-graph-with-a-cut-axis`는 축의 아래 절반을 잘라 냈다는 부정확한 예시를 세로축을 40에서 시작하는 실제 절단축 예시로 바꿨다. `ice-cream-sales-and-pool-visits`는 공통 원인이 보인다고 직접 인과를 배제할 수 있다는 과잉 결론을, 가능한 설명과 결론 보류를 구분하는 내용으로 바로잡았다.
- 81편 수정본을 `passages.json`에 병합했다. 전체 자동 검증은 14,411건 통과, lint 경고 0이며 standalone도 재생성했다(2171KB). 로컬 실경로에서 수정된 두 지문의 본문·해설 카드 렌더링과 콘솔 오류 0을 확인했다.
- 다음: 서비스워커 캐시 버전 bump 후 커밋·push 배포. 별도 품질 잔여는 41~80번 40편 보강이다.

## 2.56. 41~80번 끊기 규칙·심화 카드 보강 (2026-08-18)

- 40편 200문장을 다시 진단해, 빈 `breakRules` 13문장과 낮은 규칙 밀도를 보강했다. 실제 문장 구조에 따라 관사+명사·전치사+목적어·조동사+본동사·be동사+보어를 분리하면 안 되는 위치로 총 140건을 추가했다. 빈 규칙 문장은 0건이 됐다.
- Travel Lv3 2편(`traveling-alone-or-together`, `learning-a-new-culture-slowly`)에 빠져 있던 `insight`를 추가해 Lv3 12편 전부가 최소 1개의 심화 카드를 갖게 했다. 로컬 화면에서 새 심화 카드·자연 번역 노출과 콘솔 오류 0을 확인했다.
- 결과: 41~80번 `breakRules` 392건(문장당 1.96), 평균 chunks 2.53, Lv3 insight 미보유 0편. 자동 검증 14,413건 및 lint 경고 0, standalone 재생성(2190KB) 통과.
- 남은 품질 작업: 이 구간의 chunks 평균(2.53)은 보강 기준 2.6보다 낮다. 다음 cycle에서 2덩어리 고정 문장을 우선으로 청킹·직독직해를 개별 재구성하고, 원문·번역의 내용 감수까지 진행한다.
