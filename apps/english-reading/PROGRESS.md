...
3.1. 구 파일(src/data/sentences.json · src/core/session.js) 삭제 - 재구성으로 dead, 사용자 승인 후(delete-batch-approval).
3.2. 중학 수준 짧은 지문 2~3편 추가 제작 후 루프 체감(유추 단어·구조 문항·요지 문항·문법 이름표 포함), 확인되면 10편 안팎으로.
3.3. 복습·내 단어장 localStorage 저장 + 약점 리포트 구현(기획 8.2 나중 범위).
3.4. core/lesson·tokenize 유닛 테스트 / 루트 CLAUDE.md §1 등록표 app 반영 / SW PRECACHE apps 등재 + 캐시 bump / 실기기 터치 감도.
