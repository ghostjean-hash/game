...
1.1. **상태**: wip - v0.6 하이브리드 독해 코드 구현 완료(검증까지). 배포·폐기 파일 삭제 승인 대기.
1.2. **시작**: 2026-07-07 (허브 "app" 묶음 신설과 함께 첫 앱으로 등록).
1.3. **마지막 갱신**: 2026-07-08.
...
3.1. 배포(/web-deploy) - 사용자 확인 대기(outward-facing).
3.2. 폐기 파일 삭제 승인 - src/data/grammar-bank.json, src/core/session.js (v0.3 잔재, 대체됨).
3.3. 기획 8.2 나중 범위 - 지문 확대(코스 늘림·소재 다양화), 단어장 복습 심화(가리고 떠올리기), 읽기 통계.
