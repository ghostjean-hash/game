...
- 상태: 기획서 사용자 확정 대기. 앱 코드는 아직 v0.3 문법 스캔.

# 3. 다음 작업 후보

(v0.3 문법 스캔 후보들 - 카테고리·문장 은행 확대, 미끼 단어 - 은 2.12 v0.4 기획 전환으로 보류)

3.1. v0.4 기획서 사용자 확정 → 하이브리드 독해 구현: 스포트라이트 효과 지문 제작 → 코드 재구성(청킹·해부 카드·단어장 자산 재사용) → 전 흐름 클릭 검증 → /web-deploy 배포.
3.2. 구현 후: 지문 확대(심리학·뇌과학 소재), 단어장 복습 심화, 읽기 통계 - 기획 8.2 나중 범위.
