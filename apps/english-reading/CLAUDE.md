...
3.4. **난이도 우선 - 코스 = 난이도 레벨**(2026-07-21 사용자 지시). 학습자는 먼저 난이도(Level 1/2/3)를 고르고, 그 레벨 안에서 여러 주제의 같은 난이도 글을 읽는다. 지문에 매긴 level이 곧 축이고(한 편에 길이·구문·어휘 중 하나만 상승), 주제(topic)는 레벨 안에서 지문을 묶어 보여주는 부가 축이다. 데이터(passages.json)는 주제별 저장(courses)을 유지하고(출제·힌트용), `core/course.js:createLevelCourses`가 표시용으로만 level별로 재그룹핑한다(레벨 안은 주제별로 묶어 정렬). 지문 목록·읽기 상단 제목은 영어 `title`을 기본으로 표시하고, 읽기 상단 제목을 누르면 한글 `titleKr`을 3초간 보여 준다. 지문 목록 카드는 제목과 상태(아직 안 읽음 / 읽는 중 / N회독)만 담는다(주제 배지·완독 표시 3종은 2026-07-27 제거 - 편수가 120편으로 늘어 목록을 훑는 속도를 우선). 목록을 떠났다 돌아오면 마지막으로 보던 스크롤 위치를 복원하고, 그 코스에 첫 진입할 때만 가장 최근 완료 지문을 화면 중앙에 놓는다. 도움 노출량은 난이도가 아니라 학습자 설정. **클리어 = 한 레벨 전체 완주**이고, 개별 지문 완독은 진행률만 채우며 연출을 두지 않는다.
3.5. **순수 로직은 core/에 격리**. tokenize·course는 DOM 미의존. main.js만 DOM을 만진다. courseProgress는 done 배열 주입으로 테스트 결정성 확보.
3.6. **상태는 기기 저장**(localStorage, `createStorage("english-reading")`): `done`(완독 지문 id), `reads`(지문별 회독수), `vocab`(단어+뜻+원문+문장 해석+출처), `savedSentences`(어려운 문장: 문장 원문+문장 해석+출처 지문+문장 순서), `settings`(노출 토글), `seenIntro`(첫 안내 1회), `progress`(지문별 읽기 진행 - 문장마다 그은 선·임시 단어·검토 여부. 단어장·목록을 오가거나 앱을 껐다 켜도 복원, 회독 완료 시 해당 지문분 리셋), `listScroll`(레벨별 지문 목록의 스크롤 위치). 단어장 카드는 단어 아래 `뜻 보기` 바, 원문 문장 아래 `해석 보기` 바로 구성하며 바를 누르면 그 자리의 글자가 뜻·해석으로 바뀌고 다시 누르면 원래 보기 문구로 돌아간다. 두 상태의 최대 높이를 항상 확보해 토글 중 아래 항목이 움직이지 않게 한다(출처 제목은 목록에 표시하지 않음). 어려운 문장은 문장 끝 메뉴에서 저장/해제하며, 상단 목록에서는 문장 아래 `해석 보기` 바로만 해석을 같은 방식으로 토글한다. 앱 진입은 항상 홈(코스 목록)으로 고정한다(2026-07-16 사용자 지시, 마지막 읽던 지문 자동 복원 폐지 - 읽던 자리 표시는 지문 재진입 시 `progress`로 복원). 단어장·어려운 문장 목록의 백버튼은 읽던 지문이 있으면 그 지문으로 복귀(목록에서 왔으면 목록으로).
...
4.3. `sentence`: `{ text, chunks[{en,kr}], naturalTranslation, wordOrderPoint{title,explanation}, breakRules{allowed[{boundary,reason}],discouraged[{boundary,reason}]}, words[{word,nth?,meaning}], grammar[{label,note}], insight?{formula,why,wrong,natural} }`. **신 필드(naturalTranslation·wordOrderPoint·breakRules)는 옵셔널 - 없으면 `core/normalize.js:normalizeSentence`가 fallback을 채운다(구스키마 지문 하위호환).**
   ...
   - `breakRules`: 끊기 5등급 채점의 허용/비추천 위치. `boundary`는 **0-based 토큰 틈 번호**(b번 단어와 b+1번 단어 사이, 유효 0~토큰수-2). `allowed`=대표 경계는 아니나 끊어도 자연스러운 위치, `discouraged`=끊으면 핵심 구조가 갈려 이해를 방해하는 위치. 각 `reason` 필수(비추천은 사용자가 실제 그은 위치의 이유만 해석 카드 상단에 노출). 대표 chunks 경계를 넣거나 allowed·discouraged 중복이면 validate 실패. **문장마다 최소 1개, 보통 2~3개를 채운다**(2026-07-28 - 빈 배열이 허용되긴 하나 실제로는 거의 모든 문장에 잡을 자리가 있고, 비면 5등급 중 허용·비추천 두 등급과 이유 카드가 그 지문에서 아예 안 뜬다. 81~120번이 200문장 전부 비어 재작업했다. 현행 실측 200편 3365항목, 문장당 3.37). 채점 판정 = `core/chunking.js:gradeChunks`(recommended>allowed>discouraged>neutral, missed=안 그은 추천경계).
   ...
   - `insight`: 구조적으로 어려운 문장에만(지문당 0~3개 - 상한 3. **Lv3는 편당 1~2개, Lv2는 구조가 복잡한 문장에 0~1개를 넣고 Lv1만 0이 정당하다** - 2026-07-28, 81~120번이 Lv3 포함 40편 전부 0이라 재작업). 4필드 필수 - 검토 후 심화 카드(공식·왜·비문, 자연 해석은 naturalTranslation이 전담)로 열림(설정 토글).
...
4.6. **검증 통과 ≠ 품질 보장** (2026-07-28, 81~120번 전수 검사에서 확인). 자동 검증이 놓치는 3종은 출제·감수 때 사람이 따로 본다 - (a) `chunkViolations` 4규칙은 '한 문장을 기계적으로 2등분'해 동사와 목적어·사역보어를 가르는 끊기를 못 잡는다(`helps her remember / the words`, `makes the story / easy to recall`가 전부 통과했다). (b) breakRules·insight는 비어도 통과한다(4.3·4.3 insight 항 기준선 참조). (c) grammar `label`에 문법이 아닌 어휘·연어를 적어도 통과한다(`in the margin of`, `again`, `pass an audition`). 또한 `tools/validate-draft.mjs`는 현재 passages.json 전체와 대조하므로 **기존 지문을 같은 id로 교체하는 작업에서는 자기 자신과 id 중복이 나 항상 FAIL한다** - 그때는 교체 대상 id를 대조 목록에서 뺀 뒤 판정한다.
