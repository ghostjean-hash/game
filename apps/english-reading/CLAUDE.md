...
1.2. **상태**: 기획 v0.2 구현 완료(2026-07-07 전체 개선). 지문 3편 + 지문 고르기 + 3단계 흐름(추측→요지→정리) + 내 단어장·완료 기록(localStorage). 구 청킹 훈련 파일(src/data/sentences.json·src/core/session.js)은 dead - 삭제 승인 대기.
...
구 파일 src/core/session.js · src/data/sentences.json은 청킹 훈련 시절 잔재(미참조, 삭제 승인 대기).
