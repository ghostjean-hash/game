...
이와 별개로 `lintPassage`가 정성 자동 경고(레벨별 단어 수·문장 길이 리듬·굽은 따옴표·레벨 초과 문법·시작어 반복)를 `규칙:` 줄로 함께 출력한다. 강제 실패가 아닌 참고 경고이므로, 걸린 항목은 판단해 반영하거나 근거가 있으면 그대로 둔다(번역 정확성·자연스러움 같은 의미 판단은 lint 대상이 아니라 LLM 감수로 본다).
