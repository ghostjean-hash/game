...
## 8. 현재 상태
기획 확정, 설계 완료. 구현은 규칙(`core/`)·저장(`store/`)·브라우저 층(`platform/` · `input/`)과 화면 셋(오늘 · 운동 실행 · 종료 요약)까지 왔고 테스트 149건이 통과한다(`node tests/run-node.js`).
...
허브 등록부(`apps/_registry.json`)에도 넣지 않았다. 지금 등재하면 허브 카드가 들어가도 아무것도 못 하는 입구가 되므로, 관리 화면이 서는 판에 함께 등재한다.
