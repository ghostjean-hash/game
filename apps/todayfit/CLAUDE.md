...
## 8. 현재 상태
기획 확정, 설계 완료. 구현은 `core/` 와 `data/` 까지 끝났고 테스트 146건이 통과한다(`node tests/run-node.js`).

아직 없는 것 - `index.html` · `store/repo.js` · `render/` · `input/` · `platform/` · `styles/`. 허브 등록부(`apps/_registry.json`)에도 넣지 않았다. 화면이 생기는 시점에 함께 등재한다.
