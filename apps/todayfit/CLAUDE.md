...
## 8. 현재 상태
기획 확정, 설계 완료. 화면 열셋이 전부 섰고 테스트 213건이 통과한다(`node tests/run-node.js`).
...
관리 화면에 걸리던 기획 결정 다섯은 사용자가 확정했다(기획서 11.1 / 05_manage-screens.md 6장) - 빈 날짜에 계획 추가는 두고, 다음 달 계획은 직접 만들고, **이미 만든 계획은 덮어쓰지 않고**(생성은 빈 날짜만 채운다), 과거 기록은 손대지 않고, **월간 계획 생성은 지난 날짜를 만들지 않는다**(이번 달은 오늘부터 월말까지, 지난 달은 만들기 자체가 없다).

남은 기획 결정 셋은 기획서 11.1.1 에 있다 - 긴 일시정지 제한 · 진행 중 세션 유효 기간 · 데이터 내보내기. 셋 다 이 판의 화면과 무관하다.

## 9. 변경 이력
- 2026-09-18: 월간 계획 생성 대상 기간 확정 - 지난 날짜를 만들지 않는다(기획서 9.5 / 01_spec.md 3.7.1). 쓰지도 않던 기간이 미실시로 쌓여 달성률이 0퍼센트로 시작하던 자리를 막았다
