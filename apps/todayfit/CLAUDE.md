...
## 8.1. 진행 중인 판 (2026-09-18)
관리 화면 묶음을 만든다. 목표는 화면 개수가 아니라 **운동 등록 → 루틴 → 요일 규칙 → 월간 계획 생성 → 오늘 화면에 운동 표시가 한 바퀴 도는 것**이다(사용자 지시). 설계는 `docs/05_manage-screens.md`, 걸음 순서는 그 문서 7장.

기획 결정 넷(계획 없는 날짜에 직접 계획 · 다음 달 생성 시점 · 이미 만든 달 재생성 · 과거 기록 손 수정)은 사용자 결정 대기다. 그 자리는 비워 두고 나머지를 만든다(05_manage-screens.md 6장).
