...
2.0.1. 관리 화면이 쓰는 값 (`05_manage-screens.md` 2.5).

| 이름 | 상수 | 값 | 단위 | 쓰는 자리 |
|---|---|---|---|---|
| 이름 글자 수 상한 | `NAME_MAX_LENGTH` | 20 | 자 | 운동명 · 루틴 이름 입력 칸 |

2.0.2. 새 운동 템플릿의 처음 값 (`NEW_EXERCISE`). 새 운동을 만들면 이 값으로 시작하고 사용자가 고친다. 시간 둘은 비운 채로 시작해 앱 기본값을 따르게 한다.
