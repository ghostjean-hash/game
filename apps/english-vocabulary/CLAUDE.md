...
1.2. **상태**: 앱 기능 완성(회상형 학습·세트 선택 메뉴·세트별 진도·목록 표시 설정·못 외운 단어 모음). 콘텐츠는 **공식 2022 개정 3,000단어 제작·검수 완료**(2026-07-30, 초등 800 + 중등 1,200 + 고등 1,000). 전수 통합 검사 통과 기록은 `docs/sources/moe-2022-english/vocab-3000-final-audit.md`.
   - **앱 반영 현황**: 초등 800은 SET01~04로 반영 완료(주제·상황 기반). 중등 1,200·고등 1,000은 **세트 설계까지 완료, 앱 데이터 변환 미실행** - 설계 정본 `docs/middle-high-learning-set-design.md`, 제안 산출물 `middle-learning-set-topic-proposal-v1.json`·`high-learning-set-topic-proposal-v1.json`. 변환은 `node tools/apply-tier-topic-proposal.mjs <middle|high>`. 변환 전까지 SET05~15는 `available:false`이고 중·고 카드의 `setId`·`learningOrder`는 null이다.
   - **2026-07-29 자동 진행 금지 지시는 2026-07-30 사용자 명시 지시("앱 반영 지금 시작")로 해제됐다.** 다만 세트 구성안 확정은 사용자 결정 영역이라 변환 전 확인을 받는다.
