...
1.2. **상태**: 앱 기능 완성(회상형 학습·세트 선택 메뉴·세트별 진도·목록 표시 설정·못 외운 단어 모음·다시 안 보기 1차/2차). 콘텐츠는 **공식 2022 개정 3,000단어 제작·검수 완료**(2026-07-30, 초등 800 + 중등 1,200 + 고등 1,000). 전수 통합 검사 통과 기록은 `docs/sources/moe-2022-english/vocab-3000-final-audit.md`.
...
3.4.1. **세 번째 상태 buried = "다시 안 보기". 단계가 둘이다**(2026-08-06 사용자 지시). `progress.buriedTier`에 `BURY_TIER.KNOWN`(1차) / `BURY_TIER.MASTERED`(2차)가 들어간다.
   - **1차 = 이미 아는 단어**(banana처럼 애초에 배울 필요가 없는 것). 진입은 학습 화면 하단 "이미 아는 단어 · 1차로 제외"(`bury`). 학습 순환·복습·"못 외운 단어 모음" 전부에서 빠지고 **진도 분모에서도 제외**된다. 원칙상 학습으로 되돌리지 않는다 - 회복은 직전 1회 Undo이고, 목록의 되살리기는 기본 접힘 상태로 두어 펼쳐야만 나온다.
   - **2차 = 외워서 정리**(learned를 복습까지 졸업시킨 것). 진입은 외운 단어 목록의 "2차로 정리"(`buryLearned`) 또는 "전부 2차로 정리"(`buryAllLearned`, 확인 모달). 복습 목록에서만 빠지고 **진도 분자에는 그대로 남는다**. `unbury`하면 active가 아니라 learned로 복귀한다.
   - **진도 계산**: `stats().start` = 원본 - 1차, `stats().done` = learned + 2차(분자), `remaining` = start - done. `learned`는 "복습 목록에 남은 수"로 뜻이 좁다. 원본 수는 `stats().total`. 2차로 정리해도 완료율이 뒤로 가지 않는 것이 이 계산의 목적이므로 분자에서 빼지 말 것.
   - **v1 저장본 호환**: 단계 개념이 없던 저장본의 buried는 `normalizeTier()`가 1차로 읽는다(당시 의미와 동일). 같은 규칙이 main.js `setProgress()`에도 있으니 한쪽만 고치지 말 것.
   - 1차 버튼은 판정 버튼과 크기·위치를 달리한 하단 보조 버튼이고 **키보드 단축키를 일부러 두지 않는다**(오타로 제외되는 사고 방지).
3.5. **안전장치 1차 포함**(요청서 핵심 수정). (a) 직전 처리 1회 Undo(mark·bury 직전 스냅샷 복원, 연속 불가). (b) 외운 단어 보관함 + 수동 복습: "기억남"=learned 유지·복습시각 갱신, "모름"=active 복귀(다음 바퀴 재등장). 복습은 Undo 대상 아님. (c) 다시 안 볼 단어 목록(홈·완료 화면의 "다시 안 볼 단어 (N)")에서 개별 되살리기 - 전부 제외해 세트가 완료로 넘어간 경우에도 진입 가능. 1차 되살리기는 접힘 기본(3.4.1).
...
4.6. **검증기(tools/validate-data.mjs)**: manifest의 available 세트를 검사. error(적용 차단) = 필드 누락(pos 포함)·빈 값·허용 외 품사·id/setId 형식·id·단어 중복(세트 내/간)·대소문자 중복·앞뒤 공백·meaningKr 비배열·(strict) 세트당 200·총 3000. warning(사람 검수 대상, 차단 아님) = 활용형 의심 중복·예문에 목표 단어(활용형) 미포함·예문 과길이. 품사 균형(요청서 6장, 세트별 품사 분포)은 후속 과제.

4.7. **단어 생성 규칙(사실 근거, 앞으로 예외 없이 적용)**: 상세·정본은 `docs/vocab-master-plan.md` 2장. 요약 - (R1) 단어는 교육부 기본어휘목록 등 검증 가능 공개 출처에 실재하는 것만, 감으로 추가 금지, 출처는 `docs/sources/`에 보존. (R2) 뜻은 출처+표준 사전 대조·교정, 핵심 1~2개. (R3) 품사는 사전 기준 대표 1개. (R4) 예문만 자작 불가피(공식 예문 부재) - 짧은 문장, 반드시 사람 검수. (R5) 난이도 층으로 세트 배분. (R6) 세트 완성 시 validate-data 통과 + 사람 검수 후 available 전환, 전체 완성 시 --strict. 출처 확보는 `docs/sources/edu-elementary-800.txt`(초급 확보), 중·고층은 조사 후 보존.
