...
1.2. **상태**: 앱 기능 완성(회상형 학습·세트 선택 메뉴·세트별 진도·목록 표시 설정·못 외운 단어 모음·다시 안 보기 영구 제외). 콘텐츠는 **공식 2022 개정 3,000단어 제작·검수 완료**(2026-07-30, 초등 800 + 중등 1,200 + 고등 1,000). 전수 통합 검사 통과 기록은 `docs/sources/moe-2022-english/vocab-3000-final-audit.md`.
...
3.4.1. **세 번째 상태 buried = "다시 안 보기"**(2026-08-06 사용자 지시 - banana처럼 이미 아는 단어를 영구 제외). 학습 순환·보관함 복습·"못 외운 단어 모음"에서 모두 빠지고, 진도 분모에서도 제외된다(`stats().start` = 원본 수 - buried, 원본 수는 `stats().total`). 200단어 중 12개를 묻으면 화면은 "188개 중 …"으로 표시. 회복 경로 둘 - 직전 1회 Undo(`bury`도 mark와 같은 스냅샷 저장), 묻은 단어 목록에서 `unbury`(active 복귀, Undo 대상 아님). 버튼은 판정 버튼과 크기·위치를 달리한 하단 보조 버튼이고 **키보드 단축키를 일부러 두지 않는다**(오타로 영구 제외되는 사고 방지).
3.5. **안전장치 1차 포함**(요청서 핵심 수정). (a) 직전 처리 1회 Undo(mark·bury 직전 스냅샷 복원, 연속 불가). (b) 외운 단어 보관함 + 수동 복습: "기억남"=learned 유지·복습시각 갱신, "모름"=active 복귀(다음 바퀴 재등장). 복습은 Undo 대상 아님. (c) 묻은 단어 목록(홈·완료 화면의 "묻은 단어 (N)")에서 개별 되살리기 - 전부 묻어 세트가 완료로 넘어간 경우에도 진입 가능.
