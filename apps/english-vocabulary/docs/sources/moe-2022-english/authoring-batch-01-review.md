...
규칙 §2의 `lexicalReference`는 "위키낱말사전·Wiktionary 등 자유 라이선스 공개 자료를 복수 교차 대조"를 요구한다. 이 배치의 품사·의미는 표준 영어 사전 지식에 근거해 작성했으나, **외부 공개 자료를 실제로 조회해 항목별로 교차 대조하는 패스는 수행하지 않았다.** 200개 중 표준적이고 이견이 적은 단어가 대부분이라 오류 위험은 낮게 보지만, 규칙 문언을 문자대로 충족하지는 않는다. 필요하면 별도 대조 패스를 추가로 돌린다.
