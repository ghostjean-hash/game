...
# 0. CLAUDE.md 파일 지도 (헷갈릴 때 여기부터)

`CLAUDE.md`는 Claude Code가 폴더 진입 시 자동으로 읽는 지침 파일이다. 이름은 다 같지만 **위치가 곧 역할**이다. 헷갈리면 이 표 하나만 보면 된다.
...
핵심: 도메인 여부를 정하는 건 `./.claude/CLAUDE.md`의 `domain:` 키 하나뿐. 나머지는 그냥 범위가 다른 개발 지침이다. 이름 겹침은 Claude Code 도구 표준이라 못 바꾸지만, 역할은 위 표대로 명확히 갈린다.
