...
1.3. 앱 등록부: `apps/_registry.json` (게임 아닌 학습·도구류). `english-reading`(영어 독해, wip - 지문 200편 1000문장 전량 반영, Lv1 60 / Lv2 80 / Lv3 60) / `english-vocabulary`(영어 단어장, playable - 3,000단어 15세트 전량 반영) / `lotto`(lotto 추천번호 - 캐릭터 시드 기반 11전략 추천, wip) / `todayfit`(오늘운동 - 월 단위 운동 계획 실행·기록, wip). 세부는 `apps/<id>/CLAUDE.md`.
1.4. lotto는 2026-07-29 `games/` → `apps/` 이동(사용자 결정). 게임이 아니라 도구류라는 분류 정정. 저장 키 접두는 `lotto_` 그대로라 사용자 진행 데이터 영향 0.
1.5. `apps/todayfit`(오늘운동 TodayFit - 개인 전용 운동 계획·실행·기록)은 규칙·저장·브라우저 층과 화면 셋(오늘 · 운동 실행 · 종료 요약)까지 구현했고 등록부에도 등재했다(사용자 지시 2026-09-17, 배포해 실기기에서 보려는 목적). **달력과 관리 화면 여덟이 아직 없어 앱 안에서 운동 템플릿·루틴·계획을 만들 길이 없다** - 허브 카드로 들어가면 `오늘 예정된 운동 없음`만 보인다. 실행 화면을 보려면 저장소에 계획을 직접 심어야 한다.
