...
1.5. `apps/todayfit`(오늘운동 TodayFit - 개인 전용 운동 계획·실행·기록)은 규칙·저장·브라우저 층과 화면 셋(오늘 · 운동 실행 · 종료 요약)까지 구현했고, 달력과 관리 화면 여덟은 아직 없다. **그래서 앱 안에서 운동 템플릿·루틴·계획을 만들 길이 없어 `_registry.json`에 아직 등재하지 않았다** - 지금 등재하면 허브 카드가 들어가도 아무것도 못 하는 입구가 된다. 등재는 관리 화면이 서는 판에 함께 한다.
