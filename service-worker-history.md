# Service Worker 캐시 버전 이력

본 파일은 `service-worker.js` 헤더 주석에서 분리된 옛 버전 이력 archive. v40 이상 활성 이력은 `service-worker.js` 본체 참조.

## 1. 분리 경위

- 2026-05-16 (Sprint 073 / S73 - F4): v9~v39 = 31개 헤더 라인이 누적되어 파일 가독성 저하. v40 이상만 본체 유지, v9~v39를 본 파일로 이전. 검색 / 회귀 디버그용.

## 2. 옛 버전 이력 (v9 ~ v39)

- v9 (2026-05-03): lotto Sprint 026-032 (S17-S23) - 추천 카드 보너스 폐기 / 통계 라벨 / 출처 식별성 / 시드 분산. stale-while-revalidate라 옛 lotto 파일이 캐시되어 사용자 새로고침 후에도 옛 화면 노출되던 문제 해결.
